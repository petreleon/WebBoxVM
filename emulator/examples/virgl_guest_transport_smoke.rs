...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut steps = 0u64;
    let (mut clear_sequence, mut draw_sequence, mut texture_pair_sequence, mut vertex_color_sequence, mut texture_color_sequence, mut uniform_sequence, mut depth_sequence, mut batch_sequence, mut depth_batch_sequence, mut depth_equal_sequence, mut depth_equal_batch_sequence, mut depth_mixed_batch_sequence, mut depth_write_mask_batch_sequence) = (None, None, None, None, None, None, None, None, None, None, None, None, None); let mut texture_sequence = None;
    let (mut upload_readback, mut clear_completed) = (false, false); let (mut draw_completed, mut depth_completed, mut batch_completed, mut depth_batch_completed, mut depth_equal_completed, mut depth_equal_batch_completed, mut depth_mixed_batch_completed) = (false, false, false, false, false, false, false);
    ...
    while steps < max_steps && start.elapsed() < timeout {
        ...
        if phase == Phase::Packet && depth_mixed_batch_completed && let Some(sequence) = depth_write_mask_batch_sequence.take() { complete(&mut vm, sequence, is_depth_write_mask_batch_readback, "VGB1 depth write-mask batch")?; println!("WBGF depth-write-mask batch BGRA readback validated"); phase = Phase::Result; }
