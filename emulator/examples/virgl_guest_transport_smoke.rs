...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut steps = 0u64;
    let (mut clear_sequence, mut draw_sequence, mut texture_pair_sequence, mut vertex_color_sequence, mut texture_color_sequence, mut uniform_sequence, mut depth_sequence, mut batch_sequence, mut depth_batch_sequence) = (None, None, None, None, None, None, None, None, None);
    let mut texture_sequence = None;
    let (mut upload_readback, mut clear_completed) = (false, false);
    let (mut draw_completed, mut depth_completed, mut batch_completed) = (false, false, false);
    ...
    while steps < max_steps && start.elapsed() < timeout {
        ...
        if phase == Phase::Packet && batch_completed && let Some(sequence) = depth_batch_sequence.take() { complete(&mut vm, sequence, is_depth_batch_readback, "VGB1 depth batch")?; println!("WBGF depth-tested batch BGRA readback validated"); phase = Phase::Result; }
