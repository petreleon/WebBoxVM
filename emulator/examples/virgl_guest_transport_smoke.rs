...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (mut repeat_completed, mut linear_completed, mut texture_pair_completed, mut vertex_color_completed, mut texture_color_completed, mut uniform_completed, mut depth_vertex_color_completed, mut depth_texture_completed, mut depth_texture_color_completed) = (false, false, false, false, false, false, false, false, false);
    while steps < max_steps && start.elapsed() < timeout {
            ...
            Phase::Packet => {
                ...
                if !packet.is_empty() {
                    match virgl_packet(&packet)? {
                        VirglPacket::Clear(sequence) if clear_sequence.is_none() => { println!("VGC1 v2 resident-candidate accepted via CPU fallback: sequence {sequence}"); clear_sequence = Some(sequence); }
                        VirglPacket::Draw(sequence)
                            if clear_completed && draw_sequence.is_none() =>
                        {
                            println!("VGB1 v6 resident-candidate solid draw validated: sequence {sequence}");
                            ...
                            if draw_completed && texture_sequence.is_none() =>
                        {
                            ...
                            println!("VGM1 v2 resident-candidate {mode:?} texture validated: sequence {sequence}");
                            texture_sequence = Some((sequence, mode));
                        }
                        VirglPacket::TexturePairDraw(sequence)
                            if linear_completed && texture_pair_sequence.is_none() =>
                        {
                            println!("VGM1 v2 resident-candidate texture pair validated: sequence {sequence}");
                        ...
                        {
                            println!("VGM1 v2 resident-candidate vertex-color validated: sequence {sequence}");
                            vertex_color_sequence = Some(sequence);
                        }
                        VirglPacket::TextureColorDraw(sequence) if vertex_color_completed && texture_color_sequence.is_none() => { println!("VGM1 v2 resident-candidate texture-color validated: sequence {sequence}"); texture_color_sequence = Some(sequence); }
                        VirglPacket::UniformDraw(sequence) if texture_color_completed && uniform_sequence.is_none() => { println!("VGB1 v6 resident-candidate uniform-buffer validated: sequence {sequence}"); uniform_sequence = Some(sequence); }
                        ...
                        VirglPacket::SolidBatch(sequence) if depth_completed && batch_sequence.is_none() => { println!("VGB1 v6 resident-candidate solid-batch validated: sequence {sequence}"); batch_sequence = Some(sequence); }
                        ...
                        VirglPacket::DepthTextureDraw(sequence) if depth_vertex_color_completed && depth_texture_sequence.is_none() => { println!("VGD1 depth-texture validated: sequence {sequence}"); depth_texture_sequence = Some(sequence); } VirglPacket::DepthTextureColorDraw(sequence) if depth_texture_completed && depth_texture_color_sequence.is_none() => { println!("VGD1 depth-texture-color validated: sequence {sequence}"); depth_texture_color_sequence = Some(sequence); } VirglPacket::MaterialBatch(sequence) if depth_texture_color_completed && material_batch_sequence.is_none() => { println!("VGM1 depth-material-batch validated: sequence {sequence}"); material_batch_sequence = Some(sequence); }
        ...
        {
            complete(&mut vm, sequence, is_clear_readback, "VGC1 CPU fallback")?;
            println!("VGC1 CPU fallback completed after upload readback: sequence {sequence}");
        ...
        {
            complete(&mut vm, sequence, is_triangle_readback, "VGB1 v6 CPU fallback solid draw")?;
        ...
        if phase == Phase::Packet && linear_completed && let Some(sequence) = texture_pair_sequence.take() {
            complete(&mut vm, sequence, is_texture_pair_readback, "VGM1 v2 CPU fallback texture pair")?;
        ...
        if phase == Phase::Packet && texture_pair_completed && let Some(sequence) = vertex_color_sequence.take() {
            complete(&mut vm, sequence, is_vertex_color_readback, "VGM1 v2 CPU fallback vertex-color")?;
        ...
        if phase == Phase::Packet && vertex_color_completed && let Some(sequence) = texture_color_sequence.take() {
            complete(&mut vm, sequence, is_texture_color_readback, "VGM1 v2 CPU fallback texture-color")?;
        ...
        if phase == Phase::Packet && texture_color_completed && let Some(sequence) = uniform_sequence.take() {
            complete(&mut vm, sequence, is_uniform_readback, "VGB1 v6 CPU fallback uniform-buffer")?;
        ...
        if phase == Phase::Packet && depth_completed && let Some(sequence) = batch_sequence.take() { complete(&mut vm, sequence, is_solid_batch_readback, "VGB1 v6 CPU fallback solid batch")?; println!("WBGF ordered solid-batch BGRA readback validated"); batch_completed = true; }
        ...
        if phase == Phase::Packet && depth_vertex_color_completed && let Some(sequence) = depth_texture_sequence.take() { complete(&mut vm, sequence, is_depth_texture_readback, "VGD1 depth texture")?; println!("WBGF depth-tested texture BGRA readback validated"); depth_texture_completed = true; } if phase == Phase::Packet && depth_texture_completed && let Some(sequence) = depth_texture_color_sequence.take() { complete(&mut vm, sequence, is_depth_texture_color_readback, "VGD1 depth texture-color")?; println!("WBGF depth-tested texture-color BGRA readback validated"); depth_texture_color_completed = true; } if phase == Phase::Packet && depth_texture_color_completed && let Some(sequence) = material_batch_sequence.take() { complete(&mut vm, sequence, is_material_batch_readback, "VGM1 material batch")?; println!("WBGF depth mixed-material batch BGRA readback validated"); phase = Phase::Result; }
