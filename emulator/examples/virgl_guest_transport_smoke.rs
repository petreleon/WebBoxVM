...
use emulator::boot::BootContext; use std::{env, error::Error, fs, time::{Duration, Instant}};
use wire::*;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut steps = 0u64;
    let (mut clear_sequence, mut draw_sequence, mut texture_pair_sequence, mut vertex_color_sequence, mut texture_color_sequence, mut uniform_sequence, mut depth_sequence, mut batch_sequence, mut depth_batch_sequence, mut depth_equal_sequence) = (None, None, None, None, None, None, None, None, None, None);
    let mut texture_sequence = None;
    let (mut upload_readback, mut clear_completed) = (false, false);
    let (mut draw_completed, mut depth_completed, mut batch_completed, mut depth_batch_completed) = (false, false, false, false);
    ...
    while steps < max_steps && start.elapsed() < timeout {
        ...
        if phase == Phase::Packet && depth_batch_completed && let Some(sequence) = depth_equal_sequence.take() { complete(&mut vm, sequence, is_depth_equal_readback, "VGD1 depth equal")?; println!("WBGF depth-equal BGRA readback validated"); phase = Phase::Result; }
