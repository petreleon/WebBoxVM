...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while steps < max_steps && start.elapsed() < timeout {
        ...
        if phase == Phase::Packet && depth_write_mask_completed && let Some(sequence) = depth_vertex_color_sequence.take() { complete(&mut vm, sequence, is_vertex_color_readback, "VGD1 depth vertex-color")?; println!("WBGF depth-tested vertex-color BGRA readback validated"); phase = Phase::Result; }
