...
const VERTICES: &[u32] = &[
    0, 0x3f40_0000, 0, 0x3f80_0000, 0x3f80_0000, 0, 0, 0x3f80_0000,
    0xbf40_0000, 0xbf40_0000, 0, 0x3f80_0000, 0, 0x3f80_0000, 0, 0x3f80_0000,
    0x3f40_0000, 0xbf40_0000, 0, 0x3f80_0000, 0, 0, 0x3f800000, 0x3f80_0000,
];
const VIEWPORT: &[u32] = &[
    0x4380_0000, 0x4340_0000, 0x3f00_0000, 0x4400_0000, 0x43c0_0000, 0x3f00_0000,
];

pub(super) fn vvc1_sequence(packet: &[u8]) -> Result<u32, String> {
    if packet.len() != 192
        || packet.get(..4) != Some(b"VGD1")
        || [4, 12, 16, 20].into_iter().zip([7, 1024, 768, 3])
    ...
    {
        return Err("guest emitted an invalid vertex-color VirGL packet".into());
    }
    read_u32(packet, 8)
        .filter(|sequence| *sequence != 0)
        .ok_or_else(|| "VGD1 vertex-color packet has no nonzero sequence".into())
}

pub(crate) fn is_vertex_color_readback(packet: &[u8]) -> bool {
    frame_pixels(packet).is_some_and(|pixels| {
        let center = (384 * 1024 + 512) * 4;
        let clipped = (384 * 1024 + 400) * 4;
        pixels[..4] == [191, 128, 64, 255]
            && pixels[center..center + 4] == [64, 64, 127, 255]
