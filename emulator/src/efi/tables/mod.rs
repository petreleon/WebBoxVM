...
pub fn setup_efi_tables(
    ...
    let boot_offsets: &[u64] = &[
        0x18, 0x20, 0x28, 0x30, 0x38, 0x40, 0x48, 0x50, 0x58, 0x60, 0x68, 0x70, 0x78, 0x80, 0x88,
        0x90, 0x98, 0xA0, 0xA8, 0xB0, 0xB8, 0xC0, 0xC8, 0xD0, 0xD8, 0xE0, 0xE8, 0xF0, 0xF8, 0x100,
        0x108, 0x110, 0x118, 0x120, 0x128, 0x130, 0x138, 0x140, 0x148, 0x150, 0x158, 0x160, 0x168,
        0x170,
    ];
    ...
    write64(
        bus,
        EFI_BOOT_SERVICES_ADDR + BS_ALLOCATE_POOL_OFFSET,
        allocpool_tp,
    );
    ...
    assert!(
        ...
        "GetMemoryMap trampoline too large: {} instructions",
        memmap.len()
    );
    write_trampoline(bus, memmap_tp, &memmap);
    write64(
        bus,
        EFI_BOOT_SERVICES_ADDR + BS_GET_MEMORY_MAP_OFFSET,
        memmap_tp,
    );
    ...
    assert!(
        hp_code.len() * 4 <= LARGE_CODE_BLOCK_SIZE as usize,
        "HandleProtocol trampoline too large: {} instructions",
        hp_code.len()
    );
    ...
    let alloc_pages_insts = trampolines::build_allocate_pages_trampoline(
    ...
    super::encode::write_success_trampoline(
        bus,
