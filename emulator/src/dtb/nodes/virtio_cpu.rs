...
pub(super) fn add_cpus(builder: &mut DtbBuilder, num_cores: usize) {
    ...
    for core_id in 0..num_cores {
        let reg = u32::try_from(core_id).expect("CPU index must fit in one DT address cell");
        builder.begin_node(&format!("cpu@{core_id:x}"));
        builder.prop("device_type", b"cpu\0");
        builder.prop("compatible", b"arm,armv8\0");
        builder.prop_u32("reg", reg);
        builder.prop("enable-method", b"psci\0");
        builder.end_node();
    }
