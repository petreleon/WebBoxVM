...
pub(super) fn draw(
    ...
    if !textured::valid(vertices) || depth_values.len() * 4 != resource.pixels.len() { return false; }
    for triangle in vertices.chunks_exact(geometry::VERTICES * STRIDE) {
        let (Some(points_3d), Some(uvs)) = (geometry::positions(triangle, STRIDE), textured::uvs(triangle)) else {
            return false;
        };
        ...
        for y in bounds.2..bounds.3 {
            for x in bounds.0..bounds.1 {
                ...
                if state.compare.passes(value, *stored) {
                    ...
                    geometry::source_over(pixel, textured::sample_one(texture, textured::interpolate(uvs, weights)));
                }
            }
        }
    }
