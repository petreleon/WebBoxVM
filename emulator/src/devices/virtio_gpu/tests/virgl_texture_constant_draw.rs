...
fn fragment_texture_constant_reuses_the_texture_color_gpu_snapshot() {
    ...
    command.extend(constants([0.5, 0.5, 0.5, 1.0]));
    command.extend(draw());
    ...
    assert_eq!(
        [72, 76, 80, 84].map(|offset| read_u32(&packet, offset)),
        [Some(0.5f32.to_bits()), Some(0.5f32.to_bits()), Some(0.5f32.to_bits()), Some(1.0f32.to_bits())],
    );
    gpu.resources.get_mut(&BUFFER).unwrap().pixels.fill(0);
    gpu.resources.get_mut(&TEXTURE).unwrap().pixels.fill(0);
    let effect = gpu.pending_3d[0].effect.clone().expect("texture-constant effect");
...
fn fragment_texture_resource_constant_snapshots_before_the_gpu_acknowledgment() {
    ...
    gpu.resources.get_mut(&TEXTURE).unwrap().pixels.chunks_exact_mut(4)
        .for_each(|pixel| pixel.copy_from_slice(&[128, 128, 128, 255]));
    store(&mut gpu, [0.5, 0.5, 0.5, 1.0]);
    assert_response(&mut gpu, &mut mem, &submit(&uniform()), RESP_OK_NODATA);
    let mut command = clear([0.1, 0.2, 0.3, 1.0]);
    command.extend(draw());
    assert_response(&mut gpu, &mut mem, &submit(&command), RESP_OK_NODATA);
    let packet = gpu.take_3d_update();
    ...
    store(&mut gpu, [1.0; 4]);
    let effect = gpu.pending_3d[0].effect.clone().expect("resource texture-constant effect");
    assert!(gpu.apply_3d_effect(effect));
    let center = ((384 * 1024 + 512) * 4) as usize;
    assert_eq!(&gpu.resources[&TARGET].pixels[center..center + 4], &[64, 64, 64, 255]);
}
...
fn store(gpu: &mut super::super::VirtioGpu, color: [f32; 4]) {
    let bytes: Vec<u8> = color.into_iter().flat_map(f32::to_le_bytes).collect();
    gpu.resources.get_mut(&UNIFORM).unwrap().pixels[4..20].copy_from_slice(&bytes);
}

fn uniform() -> Vec<u32> {
    vec![word(27, 0, 5), 1, 0, 4, 16, UNIFORM]
}
