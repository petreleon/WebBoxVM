...
fn legacy_context_resource_lifecycle_is_noop_but_cannot_submit_wbg3() {
    ...
    assert_response(&mut gpu, &mem, &legacy, RESP_OK_NODATA);
    assert_response(
        &mut gpu,
        &mem,
    ...
    for command in [CMD_CTX_ATTACH_RESOURCE, CMD_CTX_DETACH_RESOURCE] {
        ...
        assert_response(&mut gpu, &mem, &request, RESP_OK_NODATA);
    }
    assert_response(
        &mut gpu,
        &mem,
...
fn old_submit_completion_does_not_touch_reused_context_id() {
    ...
    let old = deferred_submit(&mut gpu, &mem);
    attach(&mut gpu, old.sequence, old.header, OLD_RESPONSE, 11);
    assert_response(&mut gpu, &mem, &header(CMD_CTX_DESTROY), RESP_OK_NODATA);
    assert_response(&mut gpu, &mem, &context_create(), RESP_OK_NODATA);
    let new = deferred_submit(&mut gpu, &mem);
