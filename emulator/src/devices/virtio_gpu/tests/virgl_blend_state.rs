...
fn opaque_blend_uses_a_readback_replace_batch() {
    ...
    state.extend(shader_bind(11, 0)); state.extend(shader_bind(12, 1)); state.extend(opaque_state(13));
...
fn blend_mixed_batch_fails_without_committing_its_binding() {
    ...
    state.extend(opaque_state(14)); state.extend(virgl_viewport_scissor_state(15)); state.extend(vertex_state());
    assert_response(&mut gpu, &mut mem, &submit(&state), RESP_OK_NODATA); upload_vertices(&mut gpu);
