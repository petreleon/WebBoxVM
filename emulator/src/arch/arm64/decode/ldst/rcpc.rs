...
pub(in crate::arch::arm64::decode) fn decode_ldapur(raw: u32) -> Option<Instr> {
...
pub(in crate::arch::arm64::decode) fn decode_rcpc3_gpr_writeback(raw: u32) -> Option<Instr> {
...
pub(in crate::arch::arm64::decode) fn decode_rcpc3_pair(raw: u32) -> Option<Instr> {
    let op = match raw & 0x3FE0_EC00 {
        0x1940_0800 => Opcode::Ldp,
        0x1900_0800 => Opcode::Stp,
        _ => return None,
    };
    let size = match (raw >> 30) & 3 {
        2 => 4,
    ...
    let opc2 = (raw >> 12) & 0xF;
    let (cond, imm) = match (op, opc2) {
        (Opcode::Ldp, 0) => (1, (size * 2) as i64),
        (Opcode::Ldp, 1) => (0, 0),
        (Opcode::Stp, 0) => (3, -((size * 2) as i64)),
        (Opcode::Stp, 1) => (0, 0),
        _ => return None,
    };
...
pub(in crate::arch::arm64::decode) fn decode_stlur(raw: u32) -> Option<Instr> {
...
fn simm9(raw: u32) -> i64 {
    ...
    if imm & 0x100 != 0 { imm - 0x200 } else { imm }
}
