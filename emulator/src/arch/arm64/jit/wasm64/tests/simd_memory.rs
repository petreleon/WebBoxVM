...
fn call_count(module: &WasmBlockModule, index: u8) -> usize {
    module
        .bytes
        .windows(2)
        .filter(|&w| w == [opcodes::OP_CALL, index])
        .count()
}
...
fn compiles_observed_ld1_single_q_register() {
    ...
    assert_eq!(module.guest_instr_count, 1);
    assert_eq!(call_count(&module, 7), 1);
}

#[test]
fn compiles_observed_ld1_single_post_index_forms() {
    for (raw, regs, addr, pair_calls) in [
        (0x4cdf_7041, (1, 2, 0xfe), (16, 16), 1),
        (0x4cc8_7000, (0, 0, 8), (0, 16), 1),
        (0x0cdf_7004, (4, 0, 0xfe), (8, 8), 0),
    ] {
        ...
        assert_eq!(call_count(&module, 7), pair_calls);
    }
}
...
fn compiles_observed_simd_str_q_immediate() {
    ...
    assert_eq!(module.guest_instr_count, 1);
    assert_eq!(call_count(&module, 6), 1);
}
