...
fn compiles_observed_cmp_extended_register() {
    let instr = crate::arch::arm64::decode(0x6b21_02df).expect("decode observed cmp");
    assert_eq!(instr.op, Opcode::Cmp);
    assert_eq!(
        (instr.rn, instr.rm, instr.cond, instr.imm, instr.sf),
        (22, 1, 8, 0, false)
    );
...
fn compiles_observed_subs_register_result_and_flags() {
    let instr = crate::arch::arm64::decode(0x6b04_0040).expect("decode subs w0, w2, w4");
