...
impl WasmExpr {
    pub(super) fn emit_bitfield_extract(&mut self, instr: crate::arch::arm64::Instr, signed: bool) {
    ...
    pub(super) fn emit_bitfield_insert(&mut self, instr: crate::arch::arm64::Instr) {
        let size = bitfield_size(instr.sf);
        let r = instr.rm as u32;
        let s = instr.imm as u32;
        if s >= r {
