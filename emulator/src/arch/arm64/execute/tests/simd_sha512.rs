...
fn simd_sha512_three_register_ops_match_arm_round_functions() {
    ...
    assert_eq!(
        cpu.simd[2],
        sha512h_expected(d_before, cpu.simd[5], cpu.simd[6])
    );
    ...
    assert_eq!(
        cpu.simd[2],
        sha512h2_expected(d_before, cpu.simd[0], cpu.simd[3])
    );
    ...
    assert_eq!(
        cpu.simd[23],
        sha512su1_expected(d_before, cpu.simd[22], cpu.simd[7])
    );
}
...
fn sha512h_expected(w: u128, x: u128, y: u128) -> u128 {
    ...
    let high = ch(y[1], x[0], x[1])
        .wrapping_add(bs1(y[1]))
        .wrapping_add(w[1]);
    let tmp = high.wrapping_add(y[0]);
    let low = ch(tmp, y[1], x[0])
        .wrapping_add(bs1(tmp))
        .wrapping_add(w[0]);
    u64x2([low, high])
}
...
fn sha512h2_expected(w: u128, x: u128, y: u128) -> u128 {
    ...
    let x = [u64_lane(x, 0), u64_lane(x, 1)];
    let y = [u64_lane(y, 0), u64_lane(y, 1)];
    let high = maj(x[0], y[1], y[0])
        .wrapping_add(bs0(y[0]))
        .wrapping_add(w[1]);
    let low = maj(high, y[0], y[1])
        .wrapping_add(bs0(high))
        .wrapping_add(w[0]);
    u64x2([low, high])
}

fn sha512su1_expected(w: u128, x: u128, y: u128) -> u128 {
    let low = u64_lane(w, 0)
        .wrapping_add(ss1(u64_lane(x, 0)))
        .wrapping_add(u64_lane(y, 0));
    let high = u64_lane(w, 1)
        .wrapping_add(ss1(u64_lane(x, 1)))
        .wrapping_add(u64_lane(y, 1));
    u64x2([low, high])
}
