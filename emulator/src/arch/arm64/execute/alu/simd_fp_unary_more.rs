use super::*;
use crate::arch::arm64::execute::round_ties_even;

pub(in crate::arch::arm64::execute) fn exec_simd_fp_unary_more(cpu: &mut Armv8Cpu, instr: Instr) {
    ...
    cpu.simd[instr.rd as usize] = match instr.op {
        ...
        Opcode::SimdFpFrintnVec => {
            simd_fp_round_ties_even(cpu.simd[instr.rn as usize], element_size, vector_size)
        }
...
fn simd_fp_round_ties_even(value: u128, element_size: usize, vector_size: usize) -> u128 {
    match element_size {
        2 => simd_fp_elementwise_unary(value, element_size, vector_size, |bits| {
            let value = f16_to_f32(bits as u16);
            f32_to_f16_bits(round_ties_even(value as f64) as f32) as u64
        }),
        4 => simd_fp_elementwise_unary(value, element_size, vector_size, |bits| {
            (round_ties_even(f32::from_bits(bits as u32) as f64) as f32).to_bits() as u64
        }),
        8 => simd_fp_elementwise_unary(value, element_size, vector_size, |bits| {
            round_ties_even(f64::from_bits(bits)).to_bits()
        }),
        _ => 0,
    }
}
