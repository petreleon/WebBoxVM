...
pub(in crate::arch::arm64::execute) fn exec_fp_arithmetic(cpu: &mut Armv8Cpu, instr: Instr) {
    match instr.op {
...
pub(in crate::arch::arm64::execute) fn exec_fp_fnmadd(cpu: &mut Armv8Cpu, instr: Instr) {
    if instr.size == 2 {
...
pub(in crate::arch::arm64::execute) fn exec_fp_fnmsub(cpu: &mut Armv8Cpu, instr: Instr) {
    if instr.size == 2 {
        let n = f16_to_f32(read_fp_bits(cpu, instr.rn, 2) as u16);
        let m = f16_to_f32(read_fp_bits(cpu, instr.rm, 2) as u16);
        let a = f16_to_f32(read_fp_bits(cpu, instr.cond, 2) as u16);
...
pub(in crate::arch::arm64::execute) fn exec_fp_fused(
    cpu: &mut Armv8Cpu,
    instr: Instr,
...
pub(in crate::arch::arm64::execute) fn exec_fp_binary<F32, F64>(
    cpu: &mut Armv8Cpu,
    instr: Instr,
    op32: F32,
    op64: F64,
