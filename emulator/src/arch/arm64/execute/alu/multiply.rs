...
pub(in crate::arch::arm64::execute) fn exec_madd(cpu: &mut Armv8Cpu, instr: Instr) {
...
pub(in crate::arch::arm64::execute) fn exec_msub(cpu: &mut Armv8Cpu, instr: Instr) {
    let sf_src = instr.size == 0 && instr.sf;
    let n = read_reg(cpu, instr.rn, sf_src);
    let m = read_reg(cpu, instr.rm, sf_src);
    let a = read_reg(cpu, instr.cond, instr.sf);
...
pub(in crate::arch::arm64::execute) fn exec_div(cpu: &mut Armv8Cpu, instr: Instr, signed: bool) {
