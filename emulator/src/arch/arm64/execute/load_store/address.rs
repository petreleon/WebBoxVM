...
pub(in crate::arch::arm64::execute) fn compute_ldst_va(
    cpu: &Armv8Cpu,
    instr: &Instr,
) -> (u64, Option<u64>) {
...
pub(in crate::arch::arm64::execute) fn base_addr(cpu: &Armv8Cpu, rn: u8) -> u64 {
...
pub(in crate::arch::arm64::execute) fn apply_extension(val: u64, option: u8) -> u64 {
    match option {
...
pub(in crate::arch::arm64::execute) fn ldst_size(instr: &Instr) -> u8 {
    if instr.size != 0 {
        instr.size
    } else if instr.sf {
        8
...
pub(in crate::arch::arm64::execute) fn sign_extend_load(val: u64, size: u8, sf: bool) -> u64 {
    match (size, sf) {
...
pub(in crate::arch::arm64::execute) fn access_crosses_page(va: u64, size: u8) -> bool {
    (va & PAGE_OFFSET_MASK) + size as u64 > PAGE_SIZE
}

pub(in crate::arch::arm64::execute) fn access_mask(size: u8) -> u64 {
    match size {
        1 => 0xFF,
        2 => 0xFFFF,
        4 => 0xFFFF_FFFF,
...
pub(in crate::arch::arm64::execute) fn signed_ext(val: u64, size: u8) -> i64 {
    match size {
        1 => val as i8 as i64,
        2 => val as i16 as i64,
