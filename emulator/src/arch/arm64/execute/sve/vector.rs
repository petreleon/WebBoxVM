...
pub(in crate::arch::arm64::execute) fn exec_sve_movprfx(cpu: &mut Armv8Cpu, instr: Instr) {
...
pub(in crate::arch::arm64::execute) fn exec_sve_dup_gpr(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
...
pub(in crate::arch::arm64::execute) fn exec_sve_int_binary(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
    let elements = sve_vl_bytes(cpu) / element_size;
...
pub(in crate::arch::arm64::execute) fn exec_sve_logical_binary(cpu: &mut Armv8Cpu, instr: Instr) {
    let vl_bytes = sve_vl_bytes(cpu);
    let lhs = sve_read_z(cpu, instr.rn as usize);
    let rhs = sve_read_z(cpu, instr.rm as usize);
    let mut result = [0; 256];
...
pub(in crate::arch::arm64::execute) fn exec_sve_sel(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
    let vl_bytes = sve_vl_bytes(cpu);
    let elements = vl_bytes / element_size;
    let mask = cpu.sve_pred[instr.cond as usize];
