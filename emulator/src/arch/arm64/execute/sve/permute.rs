...
pub(in crate::arch::arm64::execute) fn exec_sve_zip(cpu: &mut Armv8Cpu, instr: Instr) {
...
pub(in crate::arch::arm64::execute) fn exec_sve_uzp(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
    let elements = sve_vl_bytes(cpu) / element_size;
    let pairs = elements / 2;
...
pub(in crate::arch::arm64::execute) fn exec_sve_tbl(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
