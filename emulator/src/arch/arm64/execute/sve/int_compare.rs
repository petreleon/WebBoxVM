...
pub(in crate::arch::arm64::execute) fn exec_sve_int_compare(cpu: &mut Armv8Cpu, instr: Instr) {
...
pub(in crate::arch::arm64::execute) fn exec_sve_while_predicate(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
    let vl_bytes = sve_vl_bytes(cpu);
    let elements = vl_bytes / element_size;
