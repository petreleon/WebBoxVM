...
pub(in crate::arch::arm64::execute) fn sve_fp_arith_is_immediate(instr: Instr) -> bool {
    matches!(instr.op, Opcode::SveFpAddImm | Opcode::SveFpMulImm)
        || (matches!(instr.op, Opcode::SveFpSub | Opcode::SveFpSubr) && instr.rm == 0xFF)
}

pub(in crate::arch::arm64::execute) fn exec_sve_fp_imm(cpu: &mut Armv8Cpu, instr: Instr) {
