...
mod kernel_dump_tests {
    ...
    fn dump_decompressed_kernel() {
        ...
        let dtb = build_dtb(
            ...
            None,
            None,
            Some("earlycon=pl011,0x09000000"),
        );
        ...
        for _ in 0..200_000usize {
            ...
            if cpu.regs.pc == 0x8000_CE00 {
                ...
                if dest == 0x48000000 {
                    println!(
                        "CopyMem kernel->final at step {}: src={:#x} dest={:#x} len={:#x}",
                        steps, src, dest, len
                    );
                    kernel_copied = true;
                }
                ...
                if kernel_copied && dest == 0x48000000 {
                    ...
                    println!(
                        "Dumped {} bytes to /tmp/vmlinux_decompressed",
                        vmlinux.len()
                    );
                    break;
                }
            ...
            if let Some(instr) = decode(raw) {
                ...
                println!(
                    "Unknown instruction at step {} PC={:#x} raw={:#x}",
