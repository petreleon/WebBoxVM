...
fn real_kernel_runs_past_prologue() {
    ...
    for &log_addr in &found_addresses {
        ...
        let start_addr = if log_addr > 1024 {
            log_addr - 1024
        } else {
            log_addr
        };
...
fn real_kernel_runs_past_prologue_trace() {
    ...
    use crate::efi::layout::{
        EFI_BOOT_SERVICES, EFI_LARGE_CODE, EFI_MEM_BASE, EFI_SERVICE_TRAMPOLINES,
        LARGE_CODE_STRIDE, TRAMPOLINE_STRIDE,
    };
    use crate::efi::setup_efi_tables;
    use crate::initrd::{build_cpio, load_initrd};
    use crate::loader::kernel::{KERNEL_LOAD, load_kernel};
    ...
    let mut recent: std::collections::VecDeque<String> =
        std::collections::VecDeque::with_capacity(120);
