...
pub(in crate::arm64::execute) fn simd_rotate_left_64_lanes(value: u128, shift: u32) -> u128 {
    let low = (value as u64).rotate_left(shift);
    let high = ((value >> 64) as u64).rotate_left(shift);
    ((high as u128) << 64) | low as u128
}

pub(in crate::arm64::execute) fn simd_rotate_right_64_lanes(value: u128, shift: u32) -> u128 {
    let low = (value as u64).rotate_right(shift);
    let high = ((value >> 64) as u64).rotate_right(shift);
    ((high as u128) << 64) | low as u128
}

pub(in crate::arm64::execute) fn simd_polynomial_mult(lhs: u128, rhs: u128, bits: usize) -> u128 {
    let mut out = 0u128;
    for bit in 0..bits {
        if ((rhs >> bit) & 1) != 0 {
            out ^= lhs << bit;
...
pub(in crate::arm64::execute) fn pack_u32_lanes(words: [u32; 4]) -> u128 {
    words
        .into_iter()
        .enumerate()
...
pub(in crate::arm64::execute) fn sm4_sub_word(value: u32) -> u32 {
    let mut out = 0u32;
    for byte in 0..4 {
...
pub(in crate::arm64::execute) fn sm4_linear_transform(value: u32) -> u32 {
    value
        ^ value.rotate_left(2)
...
pub(in crate::arm64::execute) fn aes_mix_columns(value: u128, inverse: bool) -> u128 {
    let bytes = aes_state_bytes(value);
    let mut out = [0u8; 16];
    for column in 0..4 {
        let base = column * 4;
...
pub(in crate::arm64::execute) fn aes_state_bytes(value: u128) -> [u8; 16] {
...
pub(in crate::arm64::execute) fn aes_state_from_bytes(bytes: [u8; 16]) -> u128 {
    let mut value = 0u128;
...
pub(in crate::arm64::execute) fn aes_mul(mut value: u8, mut factor: u8) -> u8 {
    let mut out = 0u8;
    while factor != 0 {
        if (factor & 1) != 0 {
            out ^= value;
...
pub(in crate::arm64::execute) fn aes_xtime(value: u8) -> u8 {
