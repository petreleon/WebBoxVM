pub(in crate::arm64::execute) trait FpMinMax: Copy + PartialOrd {
...
pub(in crate::arm64::execute) fn fp_max<T: FpMinMax>(lhs: T, rhs: T) -> T {
...
pub(in crate::arm64::execute) fn fp_min<T: FpMinMax>(lhs: T, rhs: T) -> T {
    if lhs.is_nan_value() || rhs.is_nan_value() {
        T::nan()
    } else if both_zero(lhs, rhs) {
...
pub(in crate::arm64::execute) fn fp_max_num<T: FpMinMax>(lhs: T, rhs: T) -> T {
...
pub(in crate::arm64::execute) fn fp_min_num<T: FpMinMax>(lhs: T, rhs: T) -> T {
    match (lhs.is_nan_value(), rhs.is_nan_value()) {
        (true, false) => rhs,
        (false, true) => lhs,
        _ => fp_min(lhs, rhs),
