use crate::arm64::{Armv8Cpu, Instr, ProcessorState};
use crate::bus::SystemBus;
use crate::constants::*;
use std::env;

pub(in crate::arm64::execute) fn exec_svc(
    cpu: &mut Armv8Cpu,
    imm16: u64,
) -> Result<(), &'static str> {
...
pub(in crate::arm64::execute) fn exec_eret(cpu: &mut Armv8Cpu) -> Result<(), &'static str> {
...
pub(in crate::arm64::execute) fn exec_brk(
    cpu: &mut Armv8Cpu,
    bus: &SystemBus,
    instr: Instr,
) -> Result<(), &'static str> {
...
pub(in crate::arm64::execute) fn exec_udf(cpu: &mut Armv8Cpu) -> Result<(), &'static str> {
    let from_lower_el = cpu.pstate.el() == 0;
    cpu.clear_exclusive();
...
fn trace_brk(cpu: &Armv8Cpu, bus: &SystemBus, pc: u64, el: u8, imm16: u64) {
    ...
    super::super::debug::dump_instructions("PC", pc, cpu, bus);
    super::super::debug::dump_instructions("LR", cpu.regs.x(LINK_REGISTER_INDEX), cpu, bus);
    super::super::debug::dump_string_pointers(cpu, bus);
    super::super::debug::dump_stack(cpu, bus);
}
