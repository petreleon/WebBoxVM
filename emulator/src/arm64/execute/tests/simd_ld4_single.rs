...
fn simd_ld4_single_loads_one_lane_into_four_registers() {
    ...
    for reg in 4..8 {
        cpu.simd[reg] = u32x4([0xaaaa_0000 + reg as u32, 0xbbbb_0000, 0xcccc_0000, 0xdddd_0000]);
    }

    execute(&mut cpu, &mut bus, decode(0x0DFF_B004).unwrap()).unwrap();

    assert_eq!(cpu.simd[4], u32x4([0xaaaa_0004, 0x1122_3344, 0xcccc_0000, 0xdddd_0000]));
    assert_eq!(cpu.simd[5], u32x4([0xaaaa_0005, 0x5566_7788, 0xcccc_0000, 0xdddd_0000]));
    assert_eq!(cpu.simd[6], u32x4([0xaaaa_0006, 0x99aa_bbcc, 0xcccc_0000, 0xdddd_0000]));
    assert_eq!(cpu.simd[7], u32x4([0xaaaa_0007, 0xddee_ff00, 0xcccc_0000, 0xdddd_0000]));
