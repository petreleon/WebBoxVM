...
fn sm4_linear_transform(value: u32) -> u32 {
    value
        ^ value.rotate_left(2)
...
fn aes_sub_shift_round(value: u128, decrypt: bool) -> u128 {
...
fn aes_mix_columns(value: u128, inverse: bool) -> u128 {
    let bytes = aes_state_bytes(value);
    let mut out = [0u8; 16];
    for column in 0..4 {
        let base = column * 4;
...
fn aes_state_bytes(value: u128) -> [u8; 16] {
...
fn aes_state_from_bytes(bytes: [u8; 16]) -> u128 {
    let mut value = 0u128;
...
fn aes_mul(mut value: u8, mut factor: u8) -> u8 {
    let mut out = 0u8;
    while factor != 0 {
        if (factor & 1) != 0 {
            out ^= value;
