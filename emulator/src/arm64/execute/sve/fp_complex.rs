...
pub(in crate::arm64::execute) fn exec_sve_fp_complex(cpu: &mut Armv8Cpu, instr: Instr) {
    ...
    for pair in 0..pairs {
        ...
        if predicate_element(&mask, real, element_size) {
            fcmla_lane(&mut result, &left, &right, real, sel_a, sel_a, neg_r, element_size);
        }
        if predicate_element(&mask, imag, element_size) {
            fcmla_lane(&mut result, &left, &right, imag, sel_a, sel_b, neg_i, element_size);
        }
    }
