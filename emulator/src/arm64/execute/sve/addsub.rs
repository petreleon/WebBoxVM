...
pub(in crate::arm64::execute) fn exec_sve_muladd_pred(cpu: &mut Armv8Cpu, instr: Instr) {
    let element_size = instr.size as usize;
    let elements = sve_vl_bytes(cpu) / element_size;
    ...
    for element in 0..elements {
        if predicate_element(&pred, element, element_size) {
            let product = sve_element(&left, element, element_size)
                .wrapping_mul(sve_element(&right, element, element_size));
