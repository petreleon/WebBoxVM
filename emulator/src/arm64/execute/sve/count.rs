pub(in crate::arm64::execute) fn sve_pred_count(pattern: u8, elements: u64) -> u64 {
    match pattern {
...
pub(in crate::arm64::execute) fn exact_pred_count(count: u64, elements: u64) -> u64 {
    if count <= elements { count } else { 0 }
}

pub(in crate::arm64::execute) fn highest_power_of_two_le(value: u64) -> u64 {
    if value == 0 {
        0
    } else {
