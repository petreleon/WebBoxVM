...
#[test]
fn compiles_observed_addp_16b_form() {
    let instr = decode(0x4e22_bc20).expect("decode addp v0.16b, v1.16b, v2.16b");
    assert_eq!(instr.op, Opcode::SimdAddp);
    assert_eq!((instr.rd, instr.rn, instr.rm), (0, 1, 2));
    assert_eq!((instr.imm, instr.size), (1, 16));

    let module = Wasm64Compiler::compile(&block(vec![instr])).expect("compile addp bytes");
