...
test "$size" -le 65536
for source in Makefile README.md demo.c draw.c index.c kms.c kms.h link.ld memory.c ops.c ops.h syscall.h texture.c texture_draw.c transfer.c transfer.h uapi.h virgl.h verify.sh; do
    lines=$(wc -l < "$source" | tr -d ' ')
    test "$lines" -le 180
done
printf 'verified %s: ELF64 AArch64 static EXEC, standard VirGL buffer/copy/upload/readback/clear/draw=%s bytes\n' \
    "$binary" "$size"
