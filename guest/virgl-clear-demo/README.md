...
source-over blend, rasterizer, viewport/scissor, inline-constant and bounded vertex/fragment-uniform triangles, interpolated vertex color, sampled texture, texture-modulated color, canonical depth-less and depth-equal triangles, and ordered non-depth plus shared-comparison depth-tested solid-draw batches,
...
`0,128,64,255` center, then validates VGB1 v2 with standard clear-one depth and near-before-far records, requiring the `0,0,128,255` center. It then validates schema 10 with standard `EQUAL`, z=1 vertices, black clear, and `128,0,0,255` BGRA, followed by VGB1 v3 shared-`EQUAL` red/blue records and `128,0,64,255` before PASS.
