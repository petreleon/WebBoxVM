# WebBoxVM standard VirGL vertex, buffer, copy, source-over, and triangle probe
...
source-over blend, triangle, and readback path. It is not Mesa, OpenGL, or Vulkan.
...
and submits one standard type-1 source-over blend object, type-4 shader
objects, type-5 vertex elements, shader binds, a generic clear, and one
non-indexed `DRAW_VBO`. It waits again and requires the scanout center pixel to read back as source-over BGRA
(`143,160,48,255`).
...
pixel is the clear color and whose center is the blended source-over result.
It accepts the marker only after guest-side vertex state, byte-buffer copy, color copy, clear, and draw
