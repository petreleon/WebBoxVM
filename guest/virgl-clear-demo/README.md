# WebBoxVM standard VirGL copy, upload, clear, and readback probe

This freestanding AArch64 Linux program is a deliberately small guest-side
proof for a conservative standard capset-1 copy, upload, clear, and readback path. It is not
...
```text
VIRGL_RESOURCE_COPY_DEMO_PASS card0 capset=1 copy=10,20,30,255:40,50,60,255 clear=64,128,191,255
```
...
only then accepts the marker emitted after the guest-side off-screen copy and
clear readback checks.

The fixed dimensions, one scanout target, and one small off-screen copy are
intentional. A mode, format, KMS, resource, or command mismatch fails at the
named stage instead of being silently treated as general graphics compatibility.
