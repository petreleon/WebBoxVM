...
static const u32 vertices[] = {
    0, 0x3f400000u, 0xbf000000u, 0x3f800000u, 0x3f800000u, 0, 0, 0x3f800000u, 0, 0x3f800000u,
    0xbf400000u, 0xbf400000u, 0xbf000000u, 0x3f800000u, 0, 0x3f800000u, 0, 0x3f800000u, 0, 0x3f800000u,
    0x3f400000u, 0xbf400000u, 0xbf000000u, 0x3f800000u, 0, 0, 0x3f800000u, 0x3f800000u, 0, 0x3f800000u,
};
...
static int upload(long fd, const struct virgl_resources *resources)
{
    static const u8 texture[] = {128, 128, 128, 255, 128, 128, 128, 255, 128, 128, 128, 255, 128, 128, 128, 255};
    struct drm_virtgpu_3d_transfer_to_host texture_transfer = {.bo_handle = resources->texture_bo, .box = {.w = 2, .h = 2, .d = 1}};
    struct drm_virtgpu_3d_transfer_to_host vertex_transfer = {.bo_handle = resources->textured_bo, .box = {.w = VIRGL_TEXTURE_COLOR_TRIANGLE_BYTES, .h = 1, .d = 1}};
    u8 *texture_map = virgl_map_buffer(fd, resources->texture_bo, VIRGL_TEXTURE_BYTES);
    u32 *vertex_map = (u32 *)virgl_map_buffer(fd, resources->textured_bo, VIRGL_TEXTURE_COLOR_TRIANGLE_BYTES);
...
static u32 stream(u32 *words, const struct virgl_resources *resources)
{
    ...
    next += append_shader(words + next, OBJECT_BASE + 2u, 0, 21, vertex_shader, sizeof(vertex_shader));
    next += append_shader(words + next, OBJECT_BASE + 3u, 1, 30, fragment_shader, sizeof(fragment_shader));
    ...
    words[next++] = VIRGL_HEADER(1, 5, 13); words[next++] = OBJECT_BASE + 6u;
