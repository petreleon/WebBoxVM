...
__attribute__((noreturn, section(".text.start"))) void _start(void) {
    ...
    if (fd < 0)
        ...
        if (virgl_upload_vertex_buffer(fd, resources.vertex_source_bo) != 0)
            ...
            if (kms != 0)
                ...
                else if ((stage = virgl_run_depth_batch(fd, &resources)) != 0) stage += 70; else if ((stage = virgl_run_depth_equal(fd, &resources)) != 0) stage += 74; else if ((stage = virgl_run_depth_equal_batch(fd, &resources)) != 0) stage += 78;
            }
        }
    }
    if (stage == 0)
    ...
    else if (stage >= 58 && stage <= 62) EMIT(fail_uniform); else if (stage >= 63 && stage <= 66) EMIT(fail_depth); else if (stage >= 67 && stage <= 70) EMIT(fail_batch); else if (stage >= 71 && stage <= 74) EMIT(fail_depth_batch); else if (stage >= 75 && stage <= 78) EMIT(fail_depth_equal); else if (stage >= 79 && stage <= 82) EMIT(fail_depth_equal_batch);
