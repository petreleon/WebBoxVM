...
static const u32 vertices[] = {
    0, 0x3f400000u, 0x3f800000u, 0x3f800000u, 0xbf400000u, 0xbf400000u, 0x3f800000u, 0x3f800000u,
    0x3f400000u, 0xbf400000u, 0x3f800000u, 0x3f800000u, 0, 0x3f400000u, 0x3f800000u, 0x3f800000u,
    0xbf400000u, 0xbf400000u, 0x3f800000u, 0x3f800000u, 0x3f400000u, 0xbf400000u, 0x3f800000u, 0x3f800000u,
};

static int upload(long fd, u32 bo); static int submit(long fd, const struct virgl_resources *resources);
...
{
    if (upload(fd, resources->depth_vertex_bo) != 0) return 1;
...
static u32 stream(u32 *words, const struct virgl_resources *resources)
{
    ...
    words[next++] = VIRGL_HEADER(1, 0, 5); words[next++] = OBJECT_BASE + 7u; words[next++] = 11; next += 3;
    words[next++] = VIRGL_HEADER(2, 0, 1); words[next++] = OBJECT_BASE + 7u;
    words[next++] = VIRGL_HEADER(7, 0, 8); words[next++] = VIRGL_CLEAR_COLOR0 | VIRGL_CLEAR_DEPTH; next += 3;
    words[next++] = 0x3f800000u; words[next++] = 0x3f800000u; next += 2;
    ...
    words[next++] = VIRGL_HEADER(12, 0, 6); words[next++] = 1; words[next++] = 0;
    words[next++] = 0; words[next++] = 0; words[next++] = 0x3f800000u; words[next++] = 0x3f000000u;
    words[next++] = VIRGL_HEADER(8, 0, 12); words[next++] = 3; words[next++] = 3; words[next++] = 4; words[next++] = 0; words[next++] = 1;
    next += 5; words[next++] = ~0u; words[next++] = 0;
    return next;
}
...
static int readback(long fd, u32 bo)
{
    ...
    struct drm_virtgpu_3d_transfer_from_host transfer = {.bo_handle = bo,
        .box = {.x = SCANOUT_WIDTH / 2u, .y = SCANOUT_HEIGHT / 2u, .w = 1, .h = 1, .d = 1}, .offset = offset};
    ...
    return pixels[offset] == 128 && pixels[offset + 1] == 0 && pixels[offset + 2] == 64 && pixels[offset + 3] == 255 ? 0 : -2;
}
