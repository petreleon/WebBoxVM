...
#define DEPTH_BYTES SCANOUT_BYTES
#define DEPTH_WORDS 192u
#define OBJECT_BASE 200u
#define VIRGL_FORMAT_Z32_FLOAT 18u
#define VIRGL_BIND_DEPTH_STENCIL 1u
...
static const u32 vertices[] = {
    0, 0x3f400000u, 0xbf000000u, 0x3f800000u, 0xbf400000u, 0xbf400000u, 0xbf000000u, 0x3f800000u,
    0x3f400000u, 0xbf400000u, 0xbf000000u, 0x3f800000u, 0, 0x3f400000u, 0x3f000000u, 0x3f800000u,
    0xbf400000u, 0xbf400000u, 0x3f000000u, 0x3f800000u, 0x3f400000u, 0xbf400000u, 0x3f000000u, 0x3f800000u,
};

static int create_depth_target(long fd, u32 *bo, u32 *resource);
...
{
    if (create_depth_target(fd, &resources->depth_bo, &resources->depth_resource) != 0)
        return -1;
    return virgl_create_triangle_buffer(fd, &resources->depth_vertex_bo, &resources->depth_vertex_resource);
}

int virgl_run_depth_triangle(long fd, const struct virgl_resources *resources)
{
    if (upload_vertices(fd, resources->depth_vertex_bo) != 0) return 1;
...
static int create_depth_target(long fd, u32 *bo, u32 *handle)
{
    struct drm_virtgpu_resource_create resource = {
        .target = VIRGL_TARGET_TEXTURE_2D, .format = VIRGL_FORMAT_Z32_FLOAT,
        .bind = VIRGL_BIND_DEPTH_STENCIL, .width = SCANOUT_WIDTH, .height = SCANOUT_HEIGHT,
        .depth = 1, .array_size = 1, .nr_samples = 1, .size = DEPTH_BYTES, .stride = SCANOUT_WIDTH * 4u,
    };

    if (sys_ioctl(fd, DRM_IOCTL_VIRTGPU_RESOURCE_CREATE, &resource) < 0 || !resource.bo_handle || !resource.res_handle)
        return -1;
    *bo = resource.bo_handle; *handle = resource.res_handle;
    return 0;
}

static int upload_vertices(long fd, u32 bo)
{
    struct drm_virtgpu_3d_transfer_to_host transfer = {.bo_handle = bo, .box = {.w = VIRGL_TRIANGLE_BYTES, .h = 1, .d = 1}};
...
static int submit(long fd, const struct virgl_resources *resources)
{
    u32 words[DEPTH_WORDS] = {0};
    u32 handles[] = {resources->scanout_bo, resources->depth_bo, resources->depth_vertex_bo};
...
static u32 stream(u32 *words, const struct virgl_resources *resources)
{
    ...
    next += append_shader(words + next, OBJECT_BASE + 2u, 0, 11, vertex_shader, sizeof(vertex_shader));
    next += append_shader(words + next, OBJECT_BASE + 3u, 1, 11, fragment_shader, sizeof(fragment_shader));
    ...
    words[next++] = 0; words[next++] = 0x3f800000u; words[next++] = 0; words[next++] = 0x3e800000u;
    ...
    words[next++] = VIRGL_HEADER(1, 5, 5); words[next++] = OBJECT_BASE + 6u;
    ...
    words[next++] = VIRGL_HEADER(6, 0, 3); words[next++] = 16; words[next++] = 0; words[next++] = resources->depth_vertex_resource;
    words[next++] = VIRGL_HEADER(1, 0, 5); words[next++] = OBJECT_BASE + 7u; words[next++] = 7; next += 3;
    words[next++] = VIRGL_HEADER(2, 0, 1); words[next++] = OBJECT_BASE + 7u;
    words[next++] = VIRGL_HEADER(7, 0, 8); words[next++] = VIRGL_CLEAR_COLOR0 | VIRGL_CLEAR_DEPTH;
    words[next++] = 0x3dcccccdu; words[next++] = 0x3e4ccccdu; words[next++] = 0x3e99999au; words[next++] = 0x3f800000u;
    words[next++] = 0x3f800000u; next += 2;
    words[next++] = VIRGL_HEADER(8, 0, 12); words[next++] = 0; words[next++] = 6; words[next++] = 4; words[next++] = 0; words[next++] = 1;
...
static int readback(long fd, u32 bo)
{
    ...
    return pixels[offset] == 58 && pixels[offset + 1] == 102 && pixels[offset + 2] == 20 && pixels[offset + 3] == 255 ? 0 : -2;
}
