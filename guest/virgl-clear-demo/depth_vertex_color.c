...
static const u32 vertices[] = {
    0, 0x3f400000u, 0xbf000000u, 0x3f800000u, 0x3f800000u, 0, 0, 0x3f800000u,
    0xbf400000u, 0xbf400000u, 0xbf000000u, 0x3f800000u, 0, 0x3f800000u, 0, 0x3f800000u,
    0x3f400000u, 0xbf400000u, 0xbf000000u, 0x3f800000u, 0, 0, 0x3f800000u, 0x3f800000u,
};
...
int virgl_run_depth_vertex_color_triangle(long fd, const struct virgl_resources *resources)
{
    ...
    if (upload(fd, resources->vertex_color_bo) != 0) return 1;
...
static int upload(long fd, u32 bo)
{
    struct drm_virtgpu_3d_transfer_to_host transfer = {
        .bo_handle = bo, .box = {.w = VIRGL_VERTEX_COLOR_TRIANGLE_BYTES, .h = 1, .d = 1},
    };
...
{
    u32 words[WORDS] = {0};
    u32 handles[] = {resources->scanout_bo, resources->depth_bo, resources->vertex_color_bo};
...
static u32 stream(u32 *words, const struct virgl_resources *resources)
{
    ...
    words[next++] = VIRGL_HEADER(6, 0, 3); words[next++] = 32; words[next++] = 0; words[next++] = resources->vertex_color_resource;
    words[next++] = VIRGL_HEADER(1, 0, 5); words[next++] = OBJECT_BASE + 7u; words[next++] = 5; next += 3;
