#!/usr/bin/env python3
"""Extract raw Vulkan registry structure without promoting its auxiliary role."""
...
ROW_FIELDS = frozenset(("requirement_kind", "container_kind", "name", "source_locator", "source_order",
                        "condition", "status", "implementation_owner", "independent_test_plan", "blocker"))
BLOCKER = "source-contract-owner-and-independent-test-plan-unassigned"
FEATURES = tuple((f"VK_{prefix}VERSION_1_{minor}", "internal" if prefix else None)
...
def row(kind: str, container: str, name: str, locator: str, order: int, source_condition: str) -> dict[str, object]:
    ...
    return {"requirement_kind": kind, "container_kind": container, "name": name, "source_locator": locator,
            "source_order": order, "condition": source_condition, "status": "blocked", "implementation_owner": None,
            "independent_test_plan": None, "blocker": BLOCKER}
...
def rows_from_raw(raw: bytes) -> list[dict[str, object]]:
    rows, order, used = [], 0, 0
    for feature in core_features(parsed_root(raw)):
...
def build_inventory(raw: bytes, identity: dict[str, object]) -> dict[str, object]:
    if identity.get("version_marker") != "VK_VERSION_1_4" or identity.get("admission") != "auxiliary-registry-metadata":
        reject("registry identity does not describe the sealed auxiliary Vulkan input")
    rows = rows_from_raw(raw)
    value = {"schema": 2, "contract": "vulkan-registry-technical-inventory-v2", "status": "blocked",
