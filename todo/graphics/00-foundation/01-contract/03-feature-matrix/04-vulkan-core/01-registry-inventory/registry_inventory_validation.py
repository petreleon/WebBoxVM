#!/usr/bin/env python3
"""Fail-closed validation for a bounded Vulkan registry fact inventory."""
...
from registry_inventory_source import RegistryError, bounded_bytes, reject, registry_identity
...
def validate_inventory(value: object, identity: dict[str, object]) -> None:
    fields = {"schema", "contract", "status", "registry", "scope", "boundaries", "rows",
              "rows_sha256", "effects"}
    if not isinstance(value, dict) or set(value) != fields or not exact(value.get("registry"), identity):
        reject("technical inventory has an invalid schema or identity")
    if (type(value.get("schema")) is not int or (value.get("schema"), value.get("contract"), value.get("status"))
            != (1, "vulkan-registry-technical-inventory-v1", "blocked")):
    ...
    for order, record in enumerate(rows, 1):
        ...
        if (kind not in KINDS or container not in CONTAINERS | {"feature"} or not isinstance(name, str)
                ...
                or (kind == "version-marker") != (container == "feature")
                or not valid_condition(record["condition"], container)):
...
def validate_scaffold(path: Path = SCAFFOLD) -> dict[str, object]:
    ...
    fields = {"schema", "contract", "status", "registry", "boundaries", "rows", "effects", "scaffold_sha256"}
    if not isinstance(value, dict) or set(value) != fields or not exact(value.get("registry"), identity):
        reject("inventory scaffold has an invalid schema or identity")
    effects = {effect: False for effect in EFFECTS}
    if (type(value.get("schema")) is not int or (value.get("schema"), value.get("contract"), value.get("status"))
            != (1, "vulkan-registry-inventory-scaffold-v1", "blocked")):
