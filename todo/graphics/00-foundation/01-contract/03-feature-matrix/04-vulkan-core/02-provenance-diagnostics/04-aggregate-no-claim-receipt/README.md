...
- [ ] Bind the active source-contract and lock identities to the registry count, order, and raw-row digest.
- [ ] Bind the Docs-map and VCTS-diagnostic receipt identities without treating either as a matrix import.
- [ ] Require missing or duplicate raw rows, stale pins, mixed receipts, and scope relabeling to fail closed.
- [ ] Keep all raw facts blocked, all claims false, CTS executions zero, and matrix-row count explicitly zero.
- [ ] Run focused positive and hostile checks plus the required local gates; preserve any unavailable external prerequisite as blocked.
- [ ] Attach the aggregate receipt without marking F03.4 complete or claiming guest, browser, CTS, certification, or performance evidence.
