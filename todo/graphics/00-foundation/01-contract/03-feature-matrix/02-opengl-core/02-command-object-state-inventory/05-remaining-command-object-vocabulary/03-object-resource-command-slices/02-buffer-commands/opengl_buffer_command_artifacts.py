#!/usr/bin/env python3
"""Sealed JSON bundle helpers for F03.2.2.5.3.2."""
...
ROOT_NAME = "opengl_buffer_command_inventory.json"
FRAGMENTS = (("binding", "opengl_buffer_command_binding.json"), ("storage", "opengl_buffer_command_storage.json"),
             ("mapping", "opengl_buffer_command_mapping.json"), ("transfer", "opengl_buffer_command_transfer.json"),
             ("query", "opengl_buffer_command_query.json"))
...
def serialized(value: object) -> str:
    ...
    if isinstance(value, dict) and isinstance(value.get("artifact_receipts"), list):
        for receipt in value["artifact_receipts"]:
            block = "\n".join("    " + line for line in json.dumps(receipt, sort_keys=True, indent=2).splitlines())
            text = text.replace(block, "    " + json.dumps(receipt, sort_keys=True, separators=(", ", ": ")))
    return text + "\n"
...
def sealed(identifier: str, declarations: list[dict[str, object]]) -> dict[str, object]:
    body = {"schema": 1, "kind": "webboxvm-opengl46-buffer-command-raw-fragment", "fragment_id": identifier,
...
def bundle(core: dict[str, object], declarations: list[dict[str, object]]) -> dict[str, dict[str, object]]:
    ...
    if [len(groups[identifier]) for identifier, _ in FRAGMENTS] != [8, 10, 8, 4, 8] or len(declarations) != 38:
...
def self_hash(value: dict[str, object], key: str, reject) -> None:
    if value.get(key) != digest({name: item for name, item in value.items() if name != key}): reject("raw artifact has a stale self hash")
...
def validate(root_path: Path, expected: dict[str, dict[str, object]], reject) -> dict[str, object]:
    wanted, visible = set(expected), {path.name for path in root_path.parent.glob("opengl_buffer_command_*.json")}
