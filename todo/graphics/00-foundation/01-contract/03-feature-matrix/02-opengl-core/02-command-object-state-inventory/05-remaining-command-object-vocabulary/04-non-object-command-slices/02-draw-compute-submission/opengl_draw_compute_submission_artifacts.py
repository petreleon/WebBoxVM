#!/usr/bin/env python3
"""Readable, self-hashed artifact bundle for F03.2.2.5.4.2."""
...
ROOT_NAME = "opengl_draw_compute_submission.json"
FRAGMENTS = (
    ("array-basic", "opengl_draw_compute_array_basic.json"),
    ("element-basic", "opengl_draw_compute_element_basic.json"),
    ("element-advanced", "opengl_draw_compute_element_advanced.json"),
    ("transform-feedback", "opengl_draw_compute_transform_feedback.json"),
    ("compute", "opengl_draw_compute_compute.json"),
)
...
def sealed(identifier: str, declarations: list[dict[str, object]]) -> dict[str, object]:
    body = {"schema": 1, "kind": "webboxvm-opengl46-draw-compute-submission-fragment", "fragment_id": identifier,
...
def bundle(core: dict[str, object], declarations: list[dict[str, object]], exclusions: list[dict[str, object]]) -> dict[str, dict[str, object]]:
    groups = {identifier: [row for row in declarations if row["fragment"] == identifier] for identifier, _ in FRAGMENTS}
    if [len(groups[identifier]) for identifier, _ in FRAGMENTS] != [7, 6, 7, 4, 2] or len(exclusions) != 2:
    ...
    body = {**core, "formal_source_exclusions": exclusions,
            "artifact_receipts": [receipt(identifier, filename, files[filename]) for identifier, filename in FRAGMENTS]}
...
def load(path: Path, reject):
    if path.is_symlink() or not path.is_file():
        reject("raw artifact must be a regular file")
    try:
        raw = path.read_bytes()
        if len(raw) > MAX_SERIALIZED:
            reject("raw artifact exceeds its serialized-size cap")
        value = json.loads(raw.decode("utf-8"), object_pairs_hook=pairs)
    except (OSError, UnicodeDecodeError, ValueError, json.JSONDecodeError) as error:
        reject(f"raw artifact cannot be read: {error}")
    if not isinstance(value, dict):
        reject("raw artifact is not a JSON object")
...
def validate(root_path: Path, expected: dict[str, dict[str, object]], reject) -> dict[str, object]:
    wanted, visible = set(expected), {path.name for path in root_path.parent.glob("opengl_draw_compute_*.json")}
    if visible != ((wanted - {ROOT_NAME}) | {root_path.name}):
        reject("raw artifact set is missing a fragment or contains an extra fragment")
    root, _ = load(root_path, reject)
    self_hash(root, "raw_inventory_sha256", reject)
    if not exact(root, expected[ROOT_NAME]):
        reject("raw root artifact is stale, partial, reordered, or promoted")
    receipts = {item.get("file"): item for item in root["artifact_receipts"] if isinstance(item, dict)}
    if set(receipts) != wanted - {ROOT_NAME}:
        reject("raw root artifact does not bind exact fragment identities")
