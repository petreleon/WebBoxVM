#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.2.1."""
...
PROBE = HERE / "opengl_source_authority.py"


def load():
    spec = importlib.util.spec_from_file_location("f0321_test_authority", PROBE)
...
class OpenGlSourceAuthorityTests(unittest.TestCase):
    ...
    def test_exact_roots_decisions_and_no_claim_state_are_sealed(self) -> None:
        ...
        self.assertEqual((value["normative_root"]["record_id"], value["full_suite_root"]["record_id"]),
                         ("opengl-46-core-spec", "opengl-cts-gl46-main"))
        self.assertEqual([item["decision"] for item in value["locator_classes"]],
                         ["derivable", "derivable", "requires-separate-admission", "requires-separate-admission"])
    ...
    def test_only_derivable_classes_consume_the_versioned_pdf_locator(self) -> None:
        locator = "opengl46-core-pdf-v1:page=2;section=1.1"
        for identifier in ("command-object-state", "limit-format"):
            with self.subTest(identifier=identifier):
                result = AUTH.consume(identifier, locator)
                self.assertEqual(result["source"]["record_id"], "opengl-46-core-spec")
        for identifier in ("shader", "extension", "opengl-gles-registry", "vulkan-registry"):
        ...
        for invalid in ("opengl46-core-pdf-v1:page=0;section=1", "glsl-460-spec:1", locator + ";extra=1"):
            with self.subTest(locator=invalid), self.assertRaisesRegex(AUTH.BoundaryError, "syntax"):
                AUTH.consume("command-object-state", invalid)
    ...
    def test_self_hash_stale_mixed_reordered_legacy_auxiliary_and_profile_spoofs_fail(self) -> None:
        edits = (
            ...
            ("profile", lambda value: value.update(profile="gles-3.2"), True, "exact active"),
        )
    ...
    def test_cli_reports_the_boundary_and_rejects_unresolved_consumption(self) -> None:
        ...
        self.assertEqual(passed.stdout.strip(), "PASS: 4 OpenGL classes; 2 derivable, 2 unavailable; matrix-incomplete")
        failed = subprocess.run([sys.executable, str(PROBE), "--consume-class", "shader", "--locator",
                                 "opengl46-core-pdf-v1:page=2;section=1.1"], capture_output=True, text=True, env=environment)
