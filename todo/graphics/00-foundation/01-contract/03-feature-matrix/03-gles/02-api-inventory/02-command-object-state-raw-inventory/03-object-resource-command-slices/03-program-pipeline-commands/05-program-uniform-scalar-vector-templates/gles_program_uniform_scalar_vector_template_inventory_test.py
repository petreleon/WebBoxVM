#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.3.5."""
...
PROBE = HERE / "gles_program_uniform_scalar_vector_template_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f03322335_gles_program_uniform_test", PROBE)
...
class ProgramUniformScalarVectorTemplateTests(unittest.TestCase):
    def live(self) -> Path:
    ...
    def test_exact_bound_template_slice(self) -> None:
        ...
        self.assertEqual((value["profile"], value["source_class"], value["raw_entry_count"]), ("gles-3.2", "command-state", 24))
        ...
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries], [(128, "7.6.1")] * 24)
        self.assertEqual([row["declaration"] for row in entries[:8]], [MAP.CATALOG.FORMAL_WINDOW[0]] * 8)
        self.assertEqual([row["declaration"] for row in entries[8:16]], [MAP.CATALOG.FORMAL_WINDOW[1]] * 8)
        self.assertEqual([row["declaration"] for row in entries[16:20]], [MAP.CATALOG.FORMAL_WINDOW[2]] * 4)
        self.assertEqual([row["declaration"] for row in entries[20:]], [MAP.CATALOG.FORMAL_WINDOW[3]] * 4)
        self.assertFalse(any("Matrix" in row["declaration"] for row in entries))
        self.assertTrue(value["raw_only"]); self.assertFalse(value["promotion_allowed"])

    def test_rejects_changed_scope_matrix_and_grammar_expansion(self) -> None:
        ...
        matrix = "void ProgramUniformMatrix{234}{f}v( uint program, int location, sizei count, boolean transpose, const T *value );"
        templates = list(MAP.CATALOG.TEMPLATES); templates[-1] = (10, matrix); changed = tuple(templates)
        with patch.object(MAP.CATALOG, "TEMPLATES", changed), patch.object(MAP.CATALOG, "SEALED_TEMPLATES_SHA256", MAP.CATALOG.template_sha256(changed)), self.assertRaises(MAP.InventoryError):
            MAP.rendered(self.live())
        with patch.object(MAP.CATALOG, "SOURCE_PAGES", (129,)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        with patch.object(MAP.GRAMMAR, "normalize", lambda forms: ["glProgramUniform1i"]), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rejects_stale_domain_route_and_grammar_order(self) -> None:
        ...
        with patch.object(MAP.CATALOG, "ROUTE", "wrong-route"), self.assertRaises(MAP.CATALOG.CatalogError):
            MAP.CATALOG.bound_family(MAP.DOMAIN_API.CHUNKS, MAP.ARTIFACT.document)
        grammar = copy.deepcopy(MAP.GRAMMAR.validate(self.live())); grammar["grammar"]["formal_templates"][5]["source_order"] = 7
    ...
    def test_rehashed_promoted_semantic_duplicate_and_symlinked_artifacts_fail(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            item_file.write_text(json.dumps(value), encoding="utf-8")
            with self.assertRaises(MAP.InventoryError): MAP.validate(self.live(), item_file)
            value = json.loads(MAP.INVENTORY.read_text(encoding="utf-8")); value["program_state"] = "fabricated"; self.rehash(value)
            item_file.write_text(json.dumps(value), encoding="utf-8")
            with self.assertRaises(MAP.InventoryError): MAP.validate(self.live(), item_file)
    ...
    def test_unavailable_substitutes_and_private_dependencies_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f03322335_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f03322335_bad_file")
    ...
    def test_fixed_loaders_ignore_and_restore_ambient_aliases(self) -> None:
        aliases = ("f03322335_gles_program_uniform_catalog", "f03322335_gles_artifact", "f03322335_gles_domain",
                   "f03322335_gles_grammar", "f03322335_gles_cache", "f03322335_gles_ledger")
        code = "\n".join((
            ...
            f"assert module.validate(Path({str(CACHE)!r}))['raw_entry_count'] == 24",
