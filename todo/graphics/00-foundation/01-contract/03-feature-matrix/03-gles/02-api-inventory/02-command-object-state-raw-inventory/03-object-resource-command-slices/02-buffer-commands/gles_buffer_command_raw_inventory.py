#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.2 GLES buffer declaration slice."""
...
CATALOG_PATH = HERE / "gles_buffer_command_raw_catalog.py"
INVENTORY = HERE / "gles_buffer_command_raw_inventory.json"
...
class InventoryError(ValueError):
    """The buffer raw declaration slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f0332232_gles_buffer_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f0332232_gles_artifact")
DOMAIN_API = private(DOMAIN, "f0332232_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f0332232_gles_grammar")
CACHE = private(CACHE_PATH, "f0332232_gles_cache")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=68;section=6"
    try:
    ...
    return source, authority, decision, manifest, domain, grammar, family_order, raw


def rendered(cache_root: Path) -> dict[str, object]:
    source, authority, decision, manifest, domain, grammar, family_order, raw = source_input(cache_root)
    try:
        facts = CATALOG.facts(raw, CATALOG.PAGES, family_order, GRAMMAR.normalize)
    except CATALOG.CatalogError as error:
        reject(str(error))
    body = {"schema": 1, "kind": "webboxvm-gles32-buffer-command-raw-inventory", "profile": CATALOG.PROFILE,
            ...
            "grammar_sha256": grammar["grammar_sha256"], "domain_families": [{"id": "buffer", "source_order": family_order}],
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected):
        reject("buffer inventory is stale, incomplete, rerouted, or promoted")
    if not all(set(row) == FACT_KEYS for row in actual["raw_entries"]):
        reject("raw buffer declaration fields are incomplete or promoted")
...
def main() -> None:
    ...
    try:
        ...
        print(json.dumps(value, indent=2, sort_keys=True) if args.emit_json else "PASS: 15 bounded GLES buffer declarations; source-only")
