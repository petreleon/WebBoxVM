#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.6.3."""
...
PROBE = HERE / "gles_vertex_array_object_lifecycle_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f03322363_gles_vertex_array_object_test", PROBE)
...
class VertexArrayObjectLifecycleTests(unittest.TestCase):
    ...
    def test_exact_bound_vertex_array_object_forms(self) -> None:
        ...
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries], [(294, "10.4"), (294, "10.4"), (295, "10.4"), (295, "10.4")])
        self.assertEqual(entries[3]["declaration"], "boolean IsVertexArray( uint array );")
        self.assertTrue(value["raw_only"]); self.assertFalse(value["promotion_allowed"])

    def test_rejects_changed_window_cross_family_return_and_normalizer_drift(self) -> None:
        ...
        changed = list(MAP.CATALOG.DECLARATIONS); changed[0] = (294, "10.4", "CreateVertexArrays", "void CreateVertexArrays( sizei n, uint *arrays );"); changed = tuple(changed)
        ...
        changed = list(MAP.CATALOG.DECLARATIONS); changed[3] = (295, "10.4", "IsVertexArray", "void IsVertexArray( uint array );"); changed = tuple(changed)
        with patch.object(MAP.CATALOG, "DECLARATIONS", changed), patch.object(MAP.CATALOG, "SEALED_DECLARATIONS_SHA256", MAP.CATALOG.sha256(changed)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rejects_draw_fence_and_typographic_header_trap(self) -> None:
        with patch.object(MAP.CATALOG, "source_window", lambda page, value: value), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        original = MAP.CATALOG.page_text
        def missing_marker(raw, page):
            return original(raw, page).replace("10.5 Drawing Commands Using Vertex Arrays", "missing marker") if page == 295 else original(raw, page)
        with patch.object(MAP.CATALOG, "page_text", missing_marker), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rehashed_promoted_duplicate_and_symlinked_artifacts_fail(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for key, item in (("reverse", None), ("profile_support", True), ("vertex_array_state", "fabricated"), ("raw_only", False)):
    ...
    def test_unavailable_substitutes_and_private_dependencies_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f03322363_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f03322363_bad_file")
    ...
    def test_fixed_loaders_ignore_aliases_and_ambient_path_extractor(self) -> None:
        aliases = ("f03322363_gles_vertex_array_object_catalog", "f03322363_gles_artifact", "f03322363_gles_domain", "f03322363_gles_grammar", "f03322363_gles_cache", "f03322363_gles_ledger")
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.dict(os.environ, {"PATH": str(fake.parent)}, clear=False): self.assertIn(MAP.CATALOG.DECLARATIONS[0][3], MAP.CATALOG.page_text(raw, 294))
