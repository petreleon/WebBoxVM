#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.3.3 GLES Uniform template slice."""
...
CATALOG_PATH = HERE / "gles_uniform_scalar_vector_template_catalog.py"
INVENTORY = HERE / "gles_uniform_scalar_vector_template_inventory.json"
...
class InventoryError(ValueError):
    """The Uniform scalar/vector template slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322333_gles_uniform_template_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322333_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322333_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322333_gles_grammar")
CACHE = private(CACHE_PATH, "f03322333_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322333_gles_ledger")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=126;section=7.6.1"
    try:
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if (value.get("raw_entry_fields") != list(ENTRY_FIELDS)
        ...
        reject("raw Uniform scalar/vector compact-row schema is incomplete or promoted")
    defaults = value["raw_entry_defaults"]
    rows, names = value.get("raw_entries"), set()
    if not isinstance(rows, list):
        reject("raw Uniform scalar/vector entries are not a list")
    result = []
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS):
            reject("raw Uniform scalar/vector compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row))
        if not isinstance(item["unprefixed_name"], str) or item["unprefixed_name"] in names:
            reject("raw Uniform scalar/vector name is duplicate or malformed")
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw Uniform scalar/vector compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    body = {"schema": 1, "kind": "webboxvm-gles32-uniform-scalar-vector-template-raw-inventory", "profile": CATALOG.PROFILE,
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected):
        reject("Uniform scalar/vector template inventory is stale, incomplete, rerouted, or promoted")
    records(actual)
...
def main() -> None:
    ...
    try:
        ...
        print(CATALOG.pretty_json(value) if args.emit_json else "PASS: 24 bounded GLES Uniform scalar/vector forms; source-only")
