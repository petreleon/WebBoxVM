#!/usr/bin/env python3
"""Validate raw-only F03.3.2.2.3.6.4 GLES transform-feedback-object declarations."""
...
CATALOG_PATH = HERE / "gles_transform_feedback_object_catalog.py"
INVENTORY = HERE / "gles_transform_feedback_object_raw_inventory.json"
...
class InventoryError(ValueError):
    """The transform-feedback-object slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322364_gles_transform_feedback_object_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322364_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322364_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322364_gles_grammar")
CACHE = private(CACHE_PATH, "f03322364_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322364_gles_ledger")
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if value.get("raw_entry_fields") != list(ENTRY_FIELDS) or set(value.get("raw_entry_defaults", {})) != DEFAULT_FIELDS:
        reject("raw transform-feedback-object compact-row schema is incomplete or promoted")
    defaults, rows, names, result = value["raw_entry_defaults"], value.get("raw_entries"), set(), []
    if not isinstance(rows, list): reject("raw transform-feedback-object entries are not a list")
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS): reject("raw transform-feedback-object compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row)); name = item["unprefixed_name"]
        if not isinstance(name, str) or name in names: reject("raw transform-feedback-object name is duplicate or malformed")
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw transform-feedback-object compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    defaults = {"family_id": "transform-feedback", "source_family_order": family_order, "derivation_class": "literal",
                "source_locator_format": "gles32-pdf-v1:page={physical_page};section={section}"}
    body = {"schema": 1, "kind": "webboxvm-gles32-transform-feedback-object-raw-inventory", "profile": CATALOG.PROFILE,
            ...
            "domain_families": [{"id": "transform-feedback", "source_order": family_order}], "raw_entry_fields": list(ENTRY_FIELDS),
            ...
            "scope_fence": "transform-feedback-object-literals-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected): reject("transform-feedback-object inventory is stale, incomplete, rerouted, or promoted")
...
def pretty_json(value: dict[str, object]) -> str:
    ...
    for position, (key, item) in enumerate(pairs):
        ...
        if not isinstance(item, list) or not all(isinstance(row, list) and len(row) == 7 for row in item): reject("transform-feedback-object inventory rows cannot be rendered")
...
def main() -> None:
    ...
    try:
        ...
        print(pretty_json(value) if args.emit_json else "PASS: 4 bounded GLES transform-feedback-object forms; source-only")
