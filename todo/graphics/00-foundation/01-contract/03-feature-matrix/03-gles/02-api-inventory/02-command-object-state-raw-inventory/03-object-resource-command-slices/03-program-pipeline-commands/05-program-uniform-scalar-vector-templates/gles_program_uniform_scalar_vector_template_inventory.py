#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.3.5 GLES ProgramUniform template slice."""
...
CATALOG_PATH = HERE / "gles_program_uniform_scalar_vector_template_catalog.py"
INVENTORY = HERE / "gles_program_uniform_scalar_vector_template_inventory.json"
...
class InventoryError(ValueError):
    """The ProgramUniform scalar/vector template slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322335_gles_program_uniform_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322335_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322335_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322335_gles_grammar")
CACHE = private(CACHE_PATH, "f03322335_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322335_gles_ledger")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=128;section=7.6.1"
    try:
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if (value.get("raw_entry_fields") != list(ENTRY_FIELDS)
        ...
        reject("raw ProgramUniform scalar/vector compact-row schema is incomplete or promoted")
    defaults = value["raw_entry_defaults"]
    rows, names = value.get("raw_entries"), set()
    if not isinstance(rows, list):
        reject("raw ProgramUniform scalar/vector entries are not a list")
    result = []
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS):
            reject("raw ProgramUniform scalar/vector compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row))
        if not isinstance(item["unprefixed_name"], str) or item["unprefixed_name"] in names:
            reject("raw ProgramUniform scalar/vector name is duplicate or malformed")
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw ProgramUniform scalar/vector compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    try:
        facts = CATALOG.facts(raw, CATALOG.PAGES, family_order, grammar, GRAMMAR.normalize)
    except CATALOG.CatalogError as error:
        reject(str(error))
    defaults = {"family_id": "program-shader", "source_family_order": family_order, "derivation_class": "template-expansion",
                "source_locator_format": "gles32-pdf-v1:page={physical_page};section={section}"}
    body = {"schema": 1, "kind": "webboxvm-gles32-program-uniform-scalar-vector-template-raw-inventory", "profile": CATALOG.PROFILE,
            ...
            "raw_only": True, "promotion_allowed": False, "scope_fence": "formal-template-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected):
        reject("ProgramUniform scalar/vector template inventory is stale, incomplete, rerouted, or promoted")
    records(actual)
...
def main() -> None:
    ...
    try:
        ...
        print(CATALOG.pretty_json(value) if args.emit_json else "PASS: 24 bounded GLES ProgramUniform scalar/vector forms; source-only")
