#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.4.2 GLES texture image/copy slice."""
...
CATALOG_PATH = HERE / "gles_texture_image_copy_subimage_catalog.py"
INVENTORY = HERE / "gles_texture_image_copy_subimage_inventory.json"
ENTRY_FIELDS = ("raw_id", "unprefixed_name", "c_name", "declaration", "physical_page", "source_page_span", "section", "source_order")
DEFAULT_FIELDS = frozenset(("family_id", "source_family_order", "derivation_class", "source_locator_format"))


class InventoryError(ValueError):
    """The texture image/copy slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322342_gles_texture_image_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322342_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322342_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322342_gles_grammar")
CACHE = private(CACHE_PATH, "f03322342_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322342_gles_ledger")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=175;section=8.5"
    try:
        authority = CACHE.SOURCE_API.authority().validate(); decision = CACHE.SOURCE_API.authority().consume("command-state", locator)["decision"]
    ...
    if (not exact(authority.get("normative_root"), source) or not exact(receipt.get("source"), source) or not exact(manifest.get("source"), source)
            or decision.get("id") != "command-state" or receipt.get("physical_pdf_pages") != CATALOG.PAGES or not exact(domain.get("source"), source)
            ...
            or ledger.get("ledger_sha256") != domain.get("unavailable_ledger_sha256")): reject("authority, cache, domain, grammar, or unavailable-ledger identity is not the exact GLES source root")
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if value.get("raw_entry_fields") != list(ENTRY_FIELDS) or set(value.get("raw_entry_defaults", {})) != DEFAULT_FIELDS: reject("raw texture image/copy compact-row schema is incomplete or promoted")
    defaults, rows, names, result = value["raw_entry_defaults"], value.get("raw_entries"), set(), []
    if not isinstance(rows, list): reject("raw texture image/copy entries are not a list")
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS): reject("raw texture image/copy compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row)); span, name = item["source_page_span"], item["unprefixed_name"]
        if not isinstance(name, str) or name in names or not isinstance(span, list) or not span or span[0] != item["physical_page"] or not all(isinstance(page, int) and page > 0 for page in span): reject("raw texture image/copy name or page span is malformed")
        names.add(name); item.update(defaults); item["source_locator"] = defaults["source_locator_format"].format(**item); result.append(item)
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)): reject("raw texture image/copy compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    defaults = {"family_id": "texture", "source_family_order": family_order, "derivation_class": "literal", "source_locator_format": "gles32-pdf-v1:page={physical_page};section={section}"}
    body = {"schema": 1, "kind": "webboxvm-gles32-texture-image-copy-subimage-raw-inventory", "profile": CATALOG.PROFILE, "source": source, "source_class": "command-state", "source_decision": decision,
            "source_authority_sha256": authority["boundary_sha256"], "source_contract_sha256": authority["source_contract_sha256"], "inventory_lock_sha256": authority["inventory_lock_sha256"], "cache_boundary_sha256": manifest["cache_boundary_sha256"],
            "physical_pdf_pages": CATALOG.PAGES, "domain_classification_sha256": domain["classification_sha256"], "grammar_sha256": grammar["grammar_sha256"], "unavailable_ledger_sha256": ledger["ledger_sha256"],
            "domain_families": [{"id": "texture", "source_order": family_order}], "raw_entry_fields": list(ENTRY_FIELDS), "raw_entry_defaults": defaults, "raw_entries": facts, "raw_entry_count": len(facts),
            "raw_entries_sha256": hashlib.sha256(canonical(facts)).hexdigest(), "raw_only": True, "promotion_allowed": False, "scope_fence": "texture-image-copy-subimage-literals-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected): reject("texture image/copy inventory is stale, incomplete, rerouted, or promoted")
...
def pretty_json(value: dict[str, object]) -> str:
    ...
    for position, (key, item) in enumerate(pairs):
        ...
        if not isinstance(item, list) or not all(isinstance(row, list) and len(row) == 8 for row in item): reject("texture image/copy inventory rows cannot be rendered")
        lines.append('  "raw_entries": [')
        for number, row in enumerate(item):
            tail = "," if number + 1 < len(item) else ""
            lines.extend((f"    [{json.dumps(row[0])}, {json.dumps(row[1])}, {json.dumps(row[2])},", f"     {json.dumps(row[3])},", f"     {json.dumps(row[4])}, {json.dumps(row[5])}, {json.dumps(row[6])}, {json.dumps(row[7])}]{tail}"))
...
def main() -> None:
    ...
    try:
        ...
        print(pretty_json(value) if args.emit_json else "PASS: 7 bounded GLES texture image/copy forms; source-only")
