#!/usr/bin/env python3
"""Validate raw-only F03.3.2.2.3.6.3 GLES vertex-array-object declarations."""
...
CATALOG_PATH = HERE / "gles_vertex_array_object_lifecycle_catalog.py"
INVENTORY = HERE / "gles_vertex_array_object_lifecycle_raw_inventory.json"
...
class InventoryError(ValueError):
    """The vertex-array-object lifecycle slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322363_gles_vertex_array_object_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322363_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322363_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322363_gles_grammar")
CACHE = private(CACHE_PATH, "f03322363_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322363_gles_ledger")
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if value.get("raw_entry_fields") != list(ENTRY_FIELDS) or set(value.get("raw_entry_defaults", {})) != DEFAULT_FIELDS:
        reject("raw vertex-array-object compact-row schema is incomplete or promoted")
    defaults, rows, names, result = value["raw_entry_defaults"], value.get("raw_entries"), set(), []
    if not isinstance(rows, list): reject("raw vertex-array-object entries are not a list")
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS): reject("raw vertex-array-object compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row)); name = item["unprefixed_name"]
        if not isinstance(name, str) or name in names: reject("raw vertex-array-object name is duplicate or malformed")
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw vertex-array-object compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    body = {"schema": 1, "kind": "webboxvm-gles32-vertex-array-object-lifecycle-raw-inventory", "profile": CATALOG.PROFILE,
            ...
            "scope_fence": "vertex-array-object-lifecycle-literals-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected): reject("vertex-array-object lifecycle inventory is stale, incomplete, rerouted, or promoted")
...
def pretty_json(value: dict[str, object]) -> str:
    ...
    for position, (key, item) in enumerate(pairs):
        ...
        if not isinstance(item, list) or not all(isinstance(row, list) and len(row) == 7 for row in item): reject("vertex-array-object inventory rows cannot be rendered")
...
def main() -> None:
    ...
    try:
        ...
        print(pretty_json(value) if args.emit_json else "PASS: 4 bounded GLES vertex-array-object forms; source-only")
