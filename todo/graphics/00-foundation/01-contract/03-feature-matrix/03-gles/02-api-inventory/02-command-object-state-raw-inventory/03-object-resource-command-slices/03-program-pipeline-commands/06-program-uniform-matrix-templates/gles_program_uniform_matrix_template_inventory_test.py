#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.3.6."""
...
PROBE = HERE / "gles_program_uniform_matrix_template_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f03322336_gles_program_uniform_matrix_test", PROBE)
...
class ProgramUniformMatrixTemplateTests(unittest.TestCase):
    def live(self) -> Path:
    ...
    def test_exact_bound_template_slice(self) -> None:
        ...
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries], [(129, "7.6.1")] * 9)
        self.assertEqual([row["declaration"] for row in entries[:3]], [MAP.CATALOG.FORMAL_WINDOW[0]] * 3)
        self.assertEqual([row["declaration"] for row in entries[3:]], [MAP.CATALOG.FORMAL_WINDOW[1]] * 6)
        self.assertTrue(all("ProgramUniformMatrix" in row["declaration"] for row in entries))
    ...
    def test_rejects_changed_scope_scalar_and_grammar_expansion(self) -> None:
        ...
        scalar = "void ProgramUniform{1234}{if}( uint program, int location, T value );"
        templates = list(MAP.CATALOG.TEMPLATES); templates[0] = (6, scalar); changed = tuple(templates)
        ...
        witness = (129, MAP.CATALOG.SECTION_WITNESS[1])
        with patch.object(MAP.CATALOG, "SECTION_WITNESS", witness), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        with patch.object(MAP.GRAMMAR, "normalize", lambda forms: ["glProgramUniformMatrix2fv"]), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        delegate = MAP.GRAMMAR.normalize
        def reordered(forms):
            result = delegate(forms)
            return result[:1] + result[2:3] + result[1:2] + result[3:] if "2x3" in forms[0][1] else result
        with patch.object(MAP.GRAMMAR, "normalize", reordered), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rejects_stale_domain_route_and_grammar_order(self) -> None:
        ...
        with patch.object(MAP.CATALOG, "ROUTE", "wrong-route"), self.assertRaises(MAP.CATALOG.CatalogError):
            MAP.CATALOG.bound_family(MAP.DOMAIN_API.CHUNKS, MAP.ARTIFACT.document)
        grammar = copy.deepcopy(MAP.GRAMMAR.validate(self.live())); grammar["grammar"]["formal_templates"][9]["source_order"] = 11
        with self.assertRaises(MAP.CATALOG.CatalogError): MAP.CATALOG.grammar_window(grammar)
        grammar = copy.deepcopy(MAP.GRAMMAR.validate(self.live())); grammar["grammar"]["formal_templates"][10]["expansion_count"] = 5
        with self.assertRaises(MAP.CATALOG.CatalogError): MAP.CATALOG.grammar_window(grammar)
    ...
    def test_unavailable_substitutes_and_private_dependencies_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f03322336_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f03322336_bad_file")
    ...
    def test_fixed_loaders_ignore_and_restore_ambient_aliases(self) -> None:
        aliases = ("f03322336_gles_program_uniform_matrix_catalog", "f03322336_gles_artifact", "f03322336_gles_domain",
                   "f03322336_gles_grammar", "f03322336_gles_cache", "f03322336_gles_ledger")
        code = "\n".join((
