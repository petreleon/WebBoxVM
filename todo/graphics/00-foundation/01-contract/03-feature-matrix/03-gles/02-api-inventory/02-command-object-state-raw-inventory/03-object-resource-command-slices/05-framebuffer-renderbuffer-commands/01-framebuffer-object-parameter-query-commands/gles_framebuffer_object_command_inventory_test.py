#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.5.1."""
...
PROBE = HERE / "gles_framebuffer_object_command_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f03322351_gles_framebuffer_object_test", PROBE)
...
class FramebufferObjectCommandTests(unittest.TestCase):
    def live(self) -> Path:
    ...
    def test_exact_bound_framebuffer_object_forms(self) -> None:
        value = MAP.validate(self.live()); entries = MAP.records(value)
        expected = ["BindFramebuffer", "GenFramebuffers", "DeleteFramebuffers", "IsFramebuffer", "FramebufferParameteri", "GetFramebufferParameteriv", "GetFramebufferAttachmentParameteriv"]
        ...
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries], [(242, "9.2"), (244, "9.2"), (245, "9.2"), (245, "9.2"), (245, "9.2.1"), (248, "9.2.3"), (248, "9.2.3")])
        self.assertEqual(entries[3]["declaration"], "boolean IsFramebuffer( uint framebuffer );")
        self.assertTrue(value["raw_only"]); self.assertFalse(value["promotion_allowed"])

    def test_rejects_changed_window_cross_family_intrusion_and_normalizer_drift(self) -> None:
        ...
        changed = list(MAP.CATALOG.DECLARATIONS); changed[0] = (242, "9.2", "BindRenderbuffer", "void BindRenderbuffer( enum target, uint renderbuffer );"); changed = tuple(changed)
        with patch.object(MAP.CATALOG, "DECLARATIONS", changed), patch.object(MAP.CATALOG, "SEALED_DECLARATIONS_SHA256", MAP.CATALOG.sha256(changed)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        with patch.object(MAP.CATALOG, "SOURCE_PAGES", (242, 244, 245)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rejects_page_245_section_boundary_crossings(self) -> None:
        for index, section in ((3, "9.2.1"), (4, "9.2")):
        ...
        def missing_marker(raw: bytes, page: int) -> str:
            return original(raw, page).replace("9.2.1 Framebuffer Object Parameters", "9.2.1 missing marker")
        with patch.object(MAP.CATALOG, "page_text", missing_marker), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())

    def test_rejects_stale_domain_route_and_family_order(self) -> None:
    ...
    def test_unavailable_substitutes_and_private_dependencies_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f03322351_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f03322351_bad_file")
    ...
    def test_fixed_loaders_ignore_and_restore_ambient_aliases(self) -> None:
        aliases = ("f03322351_gles_framebuffer_object_catalog", "f03322351_gles_artifact", "f03322351_gles_domain", "f03322351_gles_grammar", "f03322351_gles_cache", "f03322351_gles_ledger")
