...
on pp.242, 244–245, and 248. Binding behavior, parameter results, attachments, completeness, and formats stay out.
...
- [x] Extract only `BindFramebuffer`, `GenFramebuffers`, `DeleteFramebuffers`, `IsFramebuffer`, and assigned parameter/query literals.
- [x] Preserve physical pages, section boundaries, source order, and the non-`void` `IsFramebuffer` return form.
- [x] Route attachment, completeness, values, formats, limits, lifecycle behavior, and ESSL semantics out of facts.
- [x] Reject renderbuffer/attachment/status families, extensions, registry data, guessed declarations, and ESSL imports.
