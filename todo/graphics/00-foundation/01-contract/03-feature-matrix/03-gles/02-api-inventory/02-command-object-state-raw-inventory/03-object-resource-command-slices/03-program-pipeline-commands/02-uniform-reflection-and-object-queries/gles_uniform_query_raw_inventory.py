#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.3.2 GLES declaration slice."""
...
CATALOG_PATH = HERE / "gles_uniform_query_raw_catalog.py"
INVENTORY = HERE / "gles_uniform_query_raw_inventory.json"
...
class InventoryError(ValueError):
    """The uniform/query literal declaration slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322332_gles_uniform_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322332_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322332_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322332_gles_grammar")
CACHE = private(CACHE_PATH, "f03322332_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322332_gles_ledger")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=123;section=7.6"
    try:
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw uniform/query compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    body = {"schema": 1, "kind": "webboxvm-gles32-uniform-query-raw-inventory", "profile": CATALOG.PROFILE,
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected):
        reject("uniform/query literal inventory is stale, incomplete, rerouted, or promoted")
    records(actual)
...
def main() -> None:
    ...
    try:
        ...
        print(CATALOG.pretty_json(value) if args.emit_json else "PASS: 23 bounded GLES uniform/query literals; source-only")
