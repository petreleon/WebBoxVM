#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.4.4."""
...
PROBE = HERE / "gles_texture_parameter_query_template_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f03322344_gles_texture_parameter_test", PROBE)
...
class TextureParameterQueryTemplateTests(unittest.TestCase):
    def live(self) -> Path:
    ...
    def test_exact_bound_texture_parameter_query_templates(self) -> None:
        value = MAP.validate(self.live()); entries = MAP.records(value)
        expected = ["TexParameteri", "TexParameterf", "TexParameteriv", "TexParameterfv", "TexParameterIiv", "TexParameterIuiv", "GetTexParameteriv", "GetTexParameterfv", "GetTexParameterIiv", "GetTexParameterIuiv", "GetTexLevelParameteriv", "GetTexLevelParameterfv"]
        ...
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries], [(206, "8.10")] * 6 + [(209, "8.11.2")] * 4 + [(210, "8.11.3")] * 2)
        self.assertIn("uint texture", entries[4]["declaration"]); self.assertFalse(any("Sampler" in row["declaration"] for row in entries))
        self.assertTrue(value["raw_only"]); self.assertFalse(value["promotion_allowed"])

    def test_rejects_changed_window_sampler_intrusion_and_expansion_drift(self) -> None:
        windows = (tuple(MAP.CATALOG.FORMAL_WINDOW[:-1]), tuple(reversed(MAP.CATALOG.FORMAL_WINDOW)))
        for changed in windows:
            with patch.object(MAP.CATALOG, "FORMAL_WINDOW", changed), patch.object(MAP.CATALOG, "SEALED_WINDOW_SHA256", MAP.CATALOG.sha256(changed)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        changed = list(MAP.CATALOG.FORMAL_WINDOW); changed[0] = (206, "8.10", "void SamplerParameter{if}( uint sampler, enum pname, T param );"); changed = tuple(changed)
        with patch.object(MAP.CATALOG, "FORMAL_WINDOW", changed), patch.object(MAP.CATALOG, "SEALED_WINDOW_SHA256", MAP.CATALOG.sha256(changed)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        names = list(MAP.CATALOG.EXPANDED_NAMES); names[0] = tuple(reversed(names[0])); changed_names = tuple(names)
        with patch.object(MAP.CATALOG, "EXPANDED_NAMES", changed_names), patch.object(MAP.CATALOG, "SEALED_EXPANSIONS_SHA256", MAP.CATALOG.sha256(changed_names)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        with patch.object(MAP.CATALOG, "SOURCE_PAGES", (206, 209)), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
        with patch.object(MAP.GRAMMAR, "normalize", lambda forms: ["glTexParameteri"]), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rejects_stale_domain_route_and_grammar_order(self) -> None:
        ...
        grammar = copy.deepcopy(MAP.GRAMMAR.validate(self.live()))
        item = next(row for row in grammar["grammar"]["formal_templates"] if row["formal_name"] == "GetTexParameter{if}v"); item["source_order"] = 21
    ...
    def test_rehashed_promoted_semantic_duplicate_and_symlinked_artifacts_fail(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.validate(self.live(), item_file)
            value = json.loads(MAP.INVENTORY.read_text(encoding="utf-8")); value["parameter_state"] = "fabricated"; self.rehash(value); item_file.write_text(json.dumps(value), encoding="utf-8")
            with self.assertRaises(MAP.InventoryError): MAP.validate(self.live(), item_file)
    ...
    def test_unavailable_substitutes_and_private_dependencies_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f03322344_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f03322344_bad_file")
    ...
    def test_fixed_loaders_ignore_and_restore_ambient_aliases(self) -> None:
        aliases = ("f03322344_gles_texture_parameter_catalog", "f03322344_gles_artifact", "f03322344_gles_domain", "f03322344_gles_grammar", "f03322344_gles_cache", "f03322344_gles_ledger")
