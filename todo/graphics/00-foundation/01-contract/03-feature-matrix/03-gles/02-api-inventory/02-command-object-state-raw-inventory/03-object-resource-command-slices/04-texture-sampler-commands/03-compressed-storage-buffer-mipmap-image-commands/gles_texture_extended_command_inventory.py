#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.4.3 GLES texture extended command slice."""
...
CATALOG_PATH = HERE / "gles_texture_extended_command_catalog.py"
INVENTORY = HERE / "gles_texture_extended_command_inventory.json"
...
class InventoryError(ValueError):
    """The texture extended command slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322343_gles_texture_extended_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322343_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322343_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322343_gles_grammar")
CACHE = private(CACHE_PATH, "f03322343_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322343_gles_ledger")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=195;section=8.7"
    try:
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if value.get("raw_entry_fields") != list(ENTRY_FIELDS) or set(value.get("raw_entry_defaults", {})) != DEFAULT_FIELDS: reject("raw texture extended compact-row schema is incomplete or promoted")
    defaults, rows, names, result = value["raw_entry_defaults"], value.get("raw_entries"), set(), []
    if not isinstance(rows, list): reject("raw texture extended entries are not a list")
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS): reject("raw texture extended compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row)); name = item["unprefixed_name"]
        if not isinstance(name, str) or name in names: reject("raw texture extended name is duplicate or malformed")
        names.add(name); item.update(defaults); item["source_locator"] = defaults["source_locator_format"].format(**item); result.append(item)
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)): reject("raw texture extended compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    body = {"schema": 1, "kind": "webboxvm-gles32-texture-extended-command-raw-inventory", "profile": CATALOG.PROFILE, "source": source, "source_class": "command-state", "source_decision": decision,
            ...
            "raw_entries_sha256": hashlib.sha256(canonical(facts)).hexdigest(), "raw_only": True, "promotion_allowed": False, "scope_fence": "compressed-storage-buffer-mipmap-image-literals-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected): reject("texture extended inventory is stale, incomplete, rerouted, or promoted")
...
def pretty_json(value: dict[str, object]) -> str:
    ...
    for position, (key, item) in enumerate(pairs):
        ...
        if not isinstance(item, list) or not all(isinstance(row, list) and len(row) == 7 for row in item): reject("texture extended inventory rows cannot be rendered")
...
def main() -> None:
    ...
    try:
        ...
        print(pretty_json(value) if args.emit_json else "PASS: 12 bounded GLES texture extended forms; source-only")
