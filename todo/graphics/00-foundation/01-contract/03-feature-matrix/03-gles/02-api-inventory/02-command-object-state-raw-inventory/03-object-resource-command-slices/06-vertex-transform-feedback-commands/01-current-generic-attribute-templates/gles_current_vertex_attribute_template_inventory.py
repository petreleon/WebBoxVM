#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.6.1 GLES current-attribute template slice."""
...
CATALOG_PATH = HERE / "gles_current_vertex_attribute_template_catalog.py"
INVENTORY = HERE / "gles_current_vertex_attribute_template_inventory.json"
...
class InventoryError(ValueError):
    """The current-vertex-attribute template slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322361_gles_current_vertex_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322361_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322361_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322361_gles_grammar")
CACHE = private(CACHE_PATH, "f03322361_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322361_gles_ledger")
...
def source_input(cache_root: Path):
    locator = f"gles32-pdf-v1:page={CATALOG.SOURCE_PAGE};section={CATALOG.SECTION}"
    try:
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if value.get("raw_entry_fields") != list(ENTRY_FIELDS) or set(value.get("raw_entry_defaults", {})) != DEFAULT_FIELDS:
        reject("raw current-vertex-attribute compact-row schema is incomplete or promoted")
    defaults, rows, names, result = value["raw_entry_defaults"], value.get("raw_entries"), set(), []
    if not isinstance(rows, list): reject("raw current-vertex-attribute entries are not a list")
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS): reject("raw current-vertex-attribute compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row)); name = item["unprefixed_name"]
        if not isinstance(name, str) or name in names: reject("raw current-vertex-attribute name is duplicate or malformed")
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw current-vertex-attribute compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    defaults = {"family_id": "vertex-array", "source_family_order": family_order, "derivation_class": "template-expansion",
                "source_locator_format": "gles32-pdf-v1:page={physical_page};section={section}"}
    body = {"schema": 1, "kind": "webboxvm-gles32-current-vertex-attribute-template-raw-inventory", "profile": CATALOG.PROFILE,
            ...
            "scope_fence": "current-generic-attribute-templates-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected): reject("current-vertex-attribute template inventory is stale, incomplete, rerouted, or promoted")
...
def pretty_json(value: dict[str, object]) -> str:
    ...
    for position, (key, item) in enumerate(pairs):
        ...
        if not isinstance(item, list) or not all(isinstance(row, list) and len(row) == 7 for row in item): reject("current-vertex-attribute inventory rows cannot be rendered")
...
def main() -> None:
    ...
    try:
        ...
        print(pretty_json(value) if args.emit_json else "PASS: 12 bounded GLES current-vertex-attribute forms; source-only")
