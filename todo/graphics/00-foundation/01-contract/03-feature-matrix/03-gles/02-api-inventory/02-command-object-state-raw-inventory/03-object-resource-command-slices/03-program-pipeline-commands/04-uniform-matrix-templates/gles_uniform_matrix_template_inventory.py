#!/usr/bin/env python3
"""Validate the raw-only F03.3.2.2.3.3.4 GLES Uniform matrix template slice."""
...
CATALOG_PATH = HERE / "gles_uniform_matrix_template_catalog.py"
INVENTORY = HERE / "gles_uniform_matrix_template_inventory.json"
...
class InventoryError(ValueError):
    """The Uniform matrix template slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322334_gles_uniform_matrix_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322334_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322334_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322334_gles_grammar")
CACHE = private(CACHE_PATH, "f03322334_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322334_gles_ledger")
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    ...
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)):
        reject("raw Uniform matrix compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    body = {"schema": 1, "kind": "webboxvm-gles32-uniform-matrix-template-raw-inventory", "profile": CATALOG.PROFILE,
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected):
        reject("Uniform matrix template inventory is stale, incomplete, rerouted, or promoted")
    records(actual)
...
def main() -> None:
    ...
    try:
        ...
        print(CATALOG.pretty_json(value) if args.emit_json else "PASS: 9 bounded GLES Uniform matrix forms; source-only")
