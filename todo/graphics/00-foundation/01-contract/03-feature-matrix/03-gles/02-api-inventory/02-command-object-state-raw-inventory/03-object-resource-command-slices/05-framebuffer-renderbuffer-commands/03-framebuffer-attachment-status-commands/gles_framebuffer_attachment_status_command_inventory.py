#!/usr/bin/env python3
"""Validate raw-only F03.3.2.2.3.5.3 GLES framebuffer attachment/status declarations."""
...
CATALOG_PATH = HERE / "gles_framebuffer_attachment_status_command_catalog.py"
INVENTORY = HERE / "gles_framebuffer_attachment_status_command_raw_inventory.json"
...
class InventoryError(ValueError):
    """The framebuffer attachment/status command slice is not safely reproducible."""
...
CATALOG = private(CATALOG_PATH, "f03322353_gles_framebuffer_attachment_status_catalog")
ARTIFACT = private(ARTIFACT_PATH, "f03322353_gles_artifact")
DOMAIN_API = private(DOMAIN, "f03322353_gles_domain")
GRAMMAR = private(GRAMMAR_PATH, "f03322353_gles_grammar")
CACHE = private(CACHE_PATH, "f03322353_gles_cache")
LEDGER = private(LEDGER_PATH, "f03322353_gles_ledger")
...
def source_input(cache_root: Path):
    locator = "gles32-pdf-v1:page=256;section=9.2.7"
    try:
    ...
    if (source.get("profile") != CATALOG.PROFILE or not exact(authority.get("normative_root"), source) or not exact(receipt.get("source"), source) or not exact(manifest.get("source"), source) or decision.get("id") != "command-state"
            or receipt.get("physical_pdf_pages") != CATALOG.PAGES or not exact(domain.get("source"), source) or domain.get("source_class") != "command-state" or not exact(grammar.get("source"), source)
            or grammar.get("domain_classification_sha256") != domain.get("classification_sha256") or not exact(ledger.get("normative_root"), source) or ledger.get("ledger_sha256") != domain.get("unavailable_ledger_sha256")):
...
def records(value: dict[str, object]) -> list[dict[str, object]]:
    if value.get("raw_entry_fields") != list(ENTRY_FIELDS) or set(value.get("raw_entry_defaults", {})) != DEFAULT_FIELDS: reject("raw framebuffer attachment/status compact-row schema is incomplete or promoted")
    defaults, rows, names, result = value["raw_entry_defaults"], value.get("raw_entries"), set(), []
    if not isinstance(rows, list): reject("raw framebuffer attachment/status entries are not a list")
    for row in rows:
        if not isinstance(row, list) or len(row) != len(ENTRY_FIELDS): reject("raw framebuffer attachment/status compact-row shape is malformed")
        item = dict(zip(ENTRY_FIELDS, row)); name = item["unprefixed_name"]
        if not isinstance(name, str) or name in names: reject("raw framebuffer attachment/status name is duplicate or malformed")
        names.add(name); item.update(defaults); item["source_locator"] = defaults["source_locator_format"].format(**item); result.append(item)
    if [item["source_order"] for item in result] != list(range(1, len(result) + 1)): reject("raw framebuffer attachment/status compact-row order is incomplete or unstable")
    return result
...
def rendered(cache_root: Path) -> dict[str, object]:
    ...
    defaults = {"family_id": "framebuffer-renderbuffer", "source_family_order": family_order, "derivation_class": "literal", "source_locator_format": "gles32-pdf-v1:page={physical_page};section={section}"}
    body = {"schema": 1, "kind": "webboxvm-gles32-framebuffer-attachment-status-command-raw-inventory", "profile": CATALOG.PROFILE, "source": source, "source_class": "command-state", "source_decision": decision,
            ...
            "domain_families": [{"id": "framebuffer-renderbuffer", "source_order": family_order}], "raw_entry_fields": list(ENTRY_FIELDS), "raw_entry_defaults": defaults, "raw_entries": facts, "raw_entry_count": len(facts),
            "raw_entries_sha256": hashlib.sha256(canonical(facts)).hexdigest(), "raw_only": True, "promotion_allowed": False, "scope_fence": "framebuffer-attachment-status-literals-only"}
...
def validate(cache_root: Path, inventory_file: Path = INVENTORY) -> dict[str, object]:
    ...
    if not exact(actual, expected): reject("framebuffer attachment/status inventory is stale, incomplete, rerouted, or promoted")
...
def pretty_json(value: dict[str, object]) -> str:
    ...
    for position, (key, item) in enumerate(pairs):
        ...
        if not isinstance(item, list) or not all(isinstance(row, list) and len(row) == 7 for row in item): reject("framebuffer attachment/status inventory rows cannot be rendered")
...
def main() -> None:
    ...
    try:
        ...
        print(pretty_json(value) if args.emit_json else "PASS: 5 bounded GLES framebuffer-attachment/status forms; source-only")
