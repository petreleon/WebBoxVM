#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.3.3."""
...
PROBE = HERE / "gles_uniform_scalar_vector_template_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f03322333_gles_uniform_template_test", PROBE)
...
class UniformScalarVectorTemplateTests(unittest.TestCase):
    def live(self) -> Path:
    ...
    def test_exact_bound_template_slice(self) -> None:
        ...
        scalar = ["Uniform" + suffix for suffix in MAP.CATALOG.SCALAR_SUFFIXES]
        expected = scalar + [name + "v" for name in scalar]
        ...
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries], [(126, "7.6.1")] * 24)
        self.assertEqual([row["declaration"] for row in entries[:12]], [MAP.CATALOG.FORMAL_WINDOW[0]] * 12)
        self.assertEqual([row["declaration"] for row in entries[12:]], [MAP.CATALOG.FORMAL_WINDOW[1]] * 12)
        self.assertFalse(any("Matrix" in row["declaration"] or "Program" in row["declaration"] for row in entries))
    ...
    def test_rejects_changed_scope_matrix_and_grammar_expansion(self) -> None:
        ...
        templates = list(MAP.CATALOG.TEMPLATES); templates[1] = (4, MAP.CATALOG.FORMAL_WINDOW[2]); changed = tuple(templates)
        ...
        with patch.object(MAP.GRAMMAR, "normalize", lambda forms: ["glUniform1i"]), self.assertRaises(MAP.InventoryError): MAP.rendered(self.live())
    ...
    def test_rejects_stale_domain_route_and_grammar_order(self) -> None:
        ...
        with patch.object(MAP.CATALOG, "ROUTE", "wrong-route"), self.assertRaises(MAP.CATALOG.CatalogError):
            MAP.CATALOG.bound_family(MAP.DOMAIN_API.CHUNKS, MAP.ARTIFACT.document)
        grammar = copy.deepcopy(MAP.GRAMMAR.validate(self.live())); grammar["grammar"]["formal_templates"][1]["source_order"] = 3
    ...
    def test_rehashed_promoted_semantic_duplicate_and_symlinked_artifacts_fail(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            item_file.write_text(json.dumps(value), encoding="utf-8")
            with self.assertRaises(MAP.InventoryError): MAP.validate(self.live(), item_file)
            value = json.loads(MAP.INVENTORY.read_text(encoding="utf-8")); value["uniform_value"] = "fabricated"; self.rehash(value)
            item_file.write_text(json.dumps(value), encoding="utf-8")
            with self.assertRaises(MAP.InventoryError): MAP.validate(self.live(), item_file)
    ...
    def test_unavailable_substitutes_and_private_dependencies_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f03322333_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f03322333_bad_file")
    ...
    def test_fixed_loaders_ignore_and_restore_ambient_aliases(self) -> None:
        aliases = ("f03322333_gles_uniform_template_catalog", "f03322333_gles_artifact", "f03322333_gles_domain",
                   "f03322333_gles_grammar", "f03322333_gles_cache", "f03322333_gles_ledger")
        code = "\n".join((
