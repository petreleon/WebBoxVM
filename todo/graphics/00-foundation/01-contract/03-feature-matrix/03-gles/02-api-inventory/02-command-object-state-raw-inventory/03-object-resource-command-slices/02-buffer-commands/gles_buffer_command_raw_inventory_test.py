#!/usr/bin/env python3
"""Focused positive and hostile checks for F03.3.2.2.3.2."""
...
PROBE = HERE / "gles_buffer_command_raw_inventory.py"


def load():
    spec = importlib.util.spec_from_file_location("f0332232_gles_buffer_test", PROBE)
...
class BufferCommandRawInventoryTests(unittest.TestCase):
    def live(self) -> Path:
    ...
    def test_exact_bound_literal_slice(self) -> None:
        value = MAP.validate(self.live()); entries = value["raw_entries"]
        expected = ["GenBuffers", "DeleteBuffers", "IsBuffer", "BindBuffer", "BindBufferRange", "BindBufferBase",
                    "BufferData", "BufferSubData", "MapBufferRange", "FlushMappedBufferRange", "UnmapBuffer",
                    "CopyBufferSubData", "GetBufferParameteriv", "GetBufferParameteri64v", "GetBufferPointerv"]
        self.assertEqual((value["profile"], value["source_class"], value["raw_entry_count"]), ("gles-3.2", "command-state", 15))
        self.assertEqual(value["domain_families"], [{"id": "buffer", "source_order": 14}])
        self.assertEqual([row["unprefixed_name"] for row in entries], expected)
        self.assertEqual([(row["physical_page"], row["section"]) for row in entries[-3:]], [(80, "6.6")] * 3)
    ...
    def test_rejects_changed_set_and_order(self) -> None:
    ...
    def test_rejects_rerouted_section_and_nonbuffer_source(self) -> None:
        declarations = list(MAP.CATALOG.DECLARATIONS); declarations[8] = (*declarations[8][:1], "6.2", *declarations[8][2:])
        changed = tuple(declarations)
        with patch.object(MAP.CATALOG, "DECLARATIONS", changed), patch.object(MAP.CATALOG, "SEALED_DECLARATIONS_SHA256", MAP.CATALOG.declaration_sha256(changed)), self.assertRaises(MAP.InventoryError):
            MAP.rendered(self.live())
        declarations = list(MAP.CATALOG.DECLARATIONS); declarations[2] = (*declarations[2][:1], "6.1", *declarations[2][2:])
        changed = tuple(declarations)
        with patch.object(MAP.CATALOG, "DECLARATIONS", changed), patch.object(MAP.CATALOG, "SEALED_DECLARATIONS_SHA256", MAP.CATALOG.declaration_sha256(changed)), self.assertRaises(MAP.InventoryError):
            MAP.rendered(self.live())
        declarations = list(MAP.CATALOG.DECLARATIONS)
        declarations[0] = (57, "6", "GenQueries", "void GenQueries( sizei n, uint *ids );"); changed = tuple(declarations)
        with patch.object(MAP.CATALOG, "DECLARATIONS", changed), patch.object(MAP.CATALOG, "SEALED_DECLARATIONS_SHA256", MAP.CATALOG.declaration_sha256(changed)), self.assertRaises(MAP.InventoryError):
            MAP.rendered(self.live())
        family = list(MAP.CATALOG.FAMILY); family[4] = 69
    ...
    def test_rehashed_reordered_and_promoted_artifacts_fail(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            item_file = Path(temporary) / "inventory.json"
    ...
    def test_rejects_stale_duplicate_and_symlinked_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary); item_file = root / "inventory.json"
    ...
    def test_private_cache_and_fixed_loader_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary); bad = root / "bad.py"; bad.write_text("__file__ = 'unexpected.py'\n", encoding="utf-8")
            link = root / "link.py"; link.symlink_to(bad)
            with self.assertRaises(MAP.InventoryError): MAP.private(link, "f0332232_bad_link")
            with self.assertRaises(MAP.InventoryError): MAP.private(bad, "f0332232_bad_file")
    ...
    def test_fixed_loaders_ignore_and_restore_ambient_aliases(self) -> None:
        aliases = ("f0332232_gles_buffer_catalog", "f0332232_gles_artifact", "f0332232_gles_domain",
                   "f0332232_gles_grammar", "f0332232_gles_cache")
        code = "\n".join((
            ...
            f"assert module.validate(Path({str(CACHE)!r}))['raw_entry_count'] == 15",
