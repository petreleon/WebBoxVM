#!/usr/bin/env python3
"""Fail closed at the GLES 3.2 source-authority boundary."""
...
BOUNDARY = HERE / "gles_source_authority.json"
PROFILE = "gles-3.2"
LOCATOR_SYNTAX = "gles32-pdf-v1:page=<positive-decimal>;section=<section-path>"
LOCATOR = re.compile(r"^gles32-pdf-v1:page=[1-9][0-9]*;section=[1-9][0-9]*(?:\.[1-9][0-9]*)*$")
...
EXPECTED = {
    "normative-root": {"profile": PROFILE, "role": "normative-root", "record_id": "gles-32-spec",
                       ...
                       "bytes": 2198754},
    "full-suite-root": {"profile": PROFILE, "role": "full-suite-root", "record_id": "gles-cts-main",
                        ...
                        "bytes": 4284, "suite_id": "gles-cts-main",
                        "selector_path": "external/openglcts/data/gl_cts/data/mustpass/gles/khronos_mustpass/main/mustpass.xml",
                        "unfiltered": True, "selector_scope": "Khronos full root; not core-only"},
}


class BoundaryError(ValueError):
    """The GLES inventory boundary is not safe to consume."""
...
def bindings_api():
    ...
    name, previous = "f0331_role_aware_bindings", sys.modules.get("f0331_role_aware_bindings")
...
def locator_classes() -> list[dict[str, object]]:
    available = (("command-state", "GLES 3.2 core commands and state"),
                 ("limit-format", "GLES 3.2 core limits and format properties"))
    ...
    result.extend({"id": identifier, "decision": "requires-separate-admission", "availability": "unavailable",
                  ...
                  for identifier in ("shader", "precision", "extension"))
    return result
...
def checked_contract(contract: object, api) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if (not isinstance(contract, dict) or normative != EXPECTED["normative-root"]
        ...
        reject("contract does not expose the exact active GLES roots")
...
def rendered(contract: object, api) -> dict[str, object]:
    ...
    body = {"schema": 1, "kind": "webboxvm-gles-source-authority-boundary", "profile": PROFILE,
...
def consume(locator_class: str, locator: str, boundary_path: Path = BOUNDARY) -> dict[str, object]:
    ...
    if not isinstance(locator, str) or not LOCATOR.fullmatch(locator):
        reject("locator does not match the versioned GLES core PDF syntax")
...
def main() -> None:
    ...
    try:
        if args.consume_class is not None:
            ...
            print(f"PASS: {len(value['locator_classes'])} GLES classes; 2 derivable, 3 unavailable; matrix-incomplete")
