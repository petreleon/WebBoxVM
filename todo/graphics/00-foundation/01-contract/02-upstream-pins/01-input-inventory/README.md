...
the bounded `inputs/part-0001.toml` and `inputs/part-0002.toml` record files, and
[inventory.lock](inventory.lock) canonically binds their raw bytes with the root index. Every
`inputs` entry has exactly the ten required fields listed in the first checklist item. `local_cache`
is relative to the external `$XDG_CACHE_HOME`; no fetched upstream bytes belong in the repository.
The inventory revision is the raw SHA-256 of the checked lock, printed by the validator rather than
self-recorded.
