...
The canonical authority is [F02.1's manifest](../../01-input-inventory/manifest.toml), not an
upstream commit, fetched cache, or a real artifact record. This leaf validates generic record schema
only; F02.3.2 and F02.3.3 own actual ABI and generator-output bindings.
...
Actual before completion markers: provenance tests ran 9/9 and inventory tests 4/4. The test uses a
hard-coded current manifest SHA-256 and Linux UAPI ID/digest/license to prove a valid handwritten
