...
class PtraceFixtureTest(unittest.TestCase):
    def test_collector_requires_child_traceme_directional_syscalls_and_safe_snapshot(self):
        ...
        for token in ("PTRACE_TRACEME", "PTRACE_GET_SYSCALL_INFO", "PTRACE_O_TRACEFORK", "PTRACE_O_EXITKILL", "PTRACE_EVENT_FORK", "PTRACE_EVENT_EXEC", "dup2(null, STDIN_FILENO)", "PR_SET_DUMPABLE", "pre-run-snapshot", "post-exit-snapshot"):
