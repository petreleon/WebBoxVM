...
- [ ] Define raw, generated-input, rendered-output, recipe, configuration, scope, and predecessor identities.
- [ ] Preserve F02.2's 8 MiB source-member policy while imposing an explicit bounded output-artifact policy.
- [ ] Reject mutable, root-only, duplicate-path, malformed, partial, or active-consumer-looking data.
- [ ] Bind the pinned container context and two clean-run output identities without reusing fixture-only semantics.
- [ ] Add focused positive and hostile tests that keep every result unadmitted and not cutover-ready.
