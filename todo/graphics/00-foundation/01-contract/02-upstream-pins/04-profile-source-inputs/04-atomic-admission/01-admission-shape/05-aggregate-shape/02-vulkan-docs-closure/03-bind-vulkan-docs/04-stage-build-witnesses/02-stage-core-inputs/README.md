...
- [x] Descriptor-read every bounded raw and derived provider from both selected captures before accepting a canonical staged member.
- [x] Require provider bytes, selector, byte count, and SHA-256 to match the bound scope and comparison identities.
- [x] Keep raw and generated-input namespaces distinct even where a generated input overlaps a rendered-output path.
- [x] Rehash every staged member after publication and reject absent, changed, wrong-size, wrong-digest, duplicate-selector, scope-crossed, or capture-substituted data.
- [x] Add positive and hostile provider, scope, identity, cap, selector, and filesystem regressions with a compact receipt.
