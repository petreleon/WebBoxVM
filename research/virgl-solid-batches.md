...
color clear followed by two through sixteen solid, non-depth `DRAW_VBO`
commands. Each draw retains the standard state active at that exact command:
fragment constant, expanded vertices, viewport, and optional scissor.
...
A batch is only a source-over solid sequence against one target. Clear/copy
mixing, repeated clears, a depth attachment, or any non-solid batch record is
rejected before the context clone commits.
...
| --- | --- | --- |
| 0–3 | magic | `VGB1` |
| 4–27 | version, sequence, width, height, count, flags | v1; nonzero sequence; 2–16 draws; flags zero |
| 28–47 | clear RGBA and reserved depth | finite normalized color; depth exactly zero |
...
when absent, and issues `draw()` records in original order. Only a successful
...
The AArch64 guest proof clears black, then draws the same triangle with
half-alpha red and half-alpha green. At the center, the ordered BGRA result is
`0,128,64,255`. The native harness verifies the exact 264-byte two-record
`VGB1` packet, completes its one sequence, and checks that readback. Rust tests
cover parser layout, ordering, and the 16-record cap; browser tests prove two
draws occur in one source-over WebGPU pass.

## Deliberate next boundaries

- Mixed texture, vertex-color, uniform, or depth records do not batch yet.
- There is no depth/stencil, target switch, instancing, or multi-pass model in
  a batch.
