...
attachment. V2/V3 share one standard DSA comparison; v4 preserves one comparison per record. Each draw retains the standard state active at
...
functions. V2 remains a byte-compatible legacy `LESS` form; V3 carries one
shared comparison function, while V4 carries a comparison per record with matching native/CPU/WebGPU proof.
...
| --- | --- | --- |
| 0–3 | magic | `VGB1` |
| 4–27 | version, sequence, width, height, count, flags | v1 non-depth, v2 legacy `LESS`, v3 shared depth, or v4 per-record depth; nonzero sequence; 2–16 draws; v1/v2/v4 flags zero, v3 `PIPE_FUNC_*` 0–7 |
| 28–47 | clear RGBA and depth clear | finite normalized color; v1 depth exactly zero; v2/v3/v4 depth exactly one |
| each record | 60-byte count/solid/viewport/scissor, or v4 64-byte count/comparison/solid/viewport/scissor | 3–3,063 vertices; triangle-list count; normalized/fitting state |
...
maps its shared canonical comparison to one `depth24plus` pipeline, while V4 selects cached pipelines per record with depth writes. Only a successful
...
The AArch64 guest proof exercises all versions. V1 clears black, then draws
...
z=-.5 to `GREATER` at z=.5, producing `0,128,64,255`. The native harness verifies
each exact 264-byte v1/v2/v3 or 272-byte v4 two-record `VGB1` packet, completes its sequence, and checks
that readback. Rust tests cover parser layout, comparison isolation, depth ordering,
