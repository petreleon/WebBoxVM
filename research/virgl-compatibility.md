...
| Presentation | Clear; singleton shapes plus 2–16 ordered solid/vertex-color/one-/two-texture/texture-color snapshots, plus a fresh one-texture resident-source singleton with source-over or exact opaque replacement, through one WebGPU pass; exact non-depth solid DP4 `VGD1` v15 executes four matrix row dots in a WebGPU uniform; `VGB1`/`VGM1` can map final GPU color | No arbitrary shader/state, blending, or sampler state |
...
Each source position must be finite, have `x`, `y`, and `z` in `[-1, 1]`, `w == 1`, and every consecutive triple must form a nondegenerate triangle. Vertex UBO offset forms additionally snapshot `[dx, dy, 0, 0]`, with finite `dx/dy` in `[-1, 1]`, translate local copied vertices, and repeat that validation before packet construction; the generic form retains one fixed UV varying. The `DP4` form snapshots sixteen finite stage-zero constants from an inline or exact 64-byte resource binding, evaluates four row-major dot products against each copied position, requires finite positive output `w`, divides to canonical coordinates, and rejects any vertex outside the already supported clip volume; no clipping repair occurs. It keeps that transformed snapshot for CPU replay. The exact non-depth solid singleton also retains raw 16-byte positions plus the same rows for v15; browser validation repeats the projection boundary before WebGPU row-dot execution. Its zero, one, or two exact generic `MOV` outputs retain the fixed color/UV bytes while the position is replaced. The resulting `w` is one before the existing CPU packet and raster paths.
...
Rust tests prove capset bits, transactional no-clear, malformed-index, inline/resource-constant render/rejection, canonical `DP4` parse/rejection, inline/resource-backed matrix transactional rejection, raw-matrix schema-15 capture, generic-varying matrix normalization, solid and textured vertex-matrix packet coordinates, and shifted raster output,
...
cached pipelines, exact byte-identical vertex uploads, a row-major v15 matrix uniform and shader row-dot path, and material bind groups, standard singleton material and `VGB1`/`VGM1` shared/per-record depth-batch pipelines, bounded `draw(N)`, one batch render pass, padded map readback, and queue-gated completion.
...
1. Obtain native end-to-end execution of the newly wired `VGM1` guest phase once shell startup is available, then extend the validated v15 matrix uniform lane to explicitly checked generic vertex-color and texture forms rather than add exact TGSI strings.
