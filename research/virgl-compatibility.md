...
clear, standard source-over plus full-RGBA or RGB-only opaque blend states, deliberately bounded single
...
INV_SRC_ALPHA`) or blend-disabled full-RGBA/RGB-only replace words `0x78000000`/`0x38000000`, limited
...
singleton uses `VGD1`. Full-mask opaque work uses readback `VGB1` v8/v9 (non-depth/depth) or `VGM1` v4/v5;
RGB-only opaque work uses `VGB1` v10/v11 or `VGM1` v6/v7. A source-over solid-only 2–16 sequence uses `VGB1` v1 when non-depth,
...
Solid color, interpolated vertex color, sampled texels, and texture-times-interpolated-color use source-over unless a uniform full-RGBA or RGB-only opaque mode is selected.
