...
| VirGL stream | Surface/framebuffer, bounded normalized TGSI shapes including canonical vertex `DP4`, vertex/index/sampler state, inline/resource constants, blend/rasterizer, viewport/scissor, clear, and `DRAW_VBO` | No arbitrary TGSI or fixed-function state |
| Presentation | Clear; singleton shapes plus 2–16 ordered solid/vertex-color/one-/two-texture/texture-color snapshots, plus a fresh one-texture resident-source singleton with source-over or exact opaque replacement, through one WebGPU pass; exact non-depth DP4 `VGD1` v15 solid, v16 generic-RGBA, v17 generic-UV one-texture, and v18 generic-UV two-texture lanes execute four matrix row dots in WebGPU; `VGB1`/`VGM1` can map final GPU color | No arbitrary shader/state, blending, or sampler state |
...
and its primitive mask contains `PIPE_PRIM_TRIANGLES` (bit 4), `PIPE_PRIM_TRIANGLE_STRIP` (bit 5), and `PIPE_PRIM_TRIANGLE_FAN` (bit 6). No GLSL feature level is advertised; the UBO bit and `max_uniform_blocks` are one only for canonical stage-slot-zero color, XY-offset, and four-row vertex-matrix paths.
...
`SET_CONSTANT_BUFFER` accepts either fragment stage 1, slot zero, and exactly four finite normalized inline f32 values, or vertex stage 0, slot zero, and exactly sixteen finite inline f32 matrix values (an empty binding clears either); command 27 accepts fragment stage 1 or vertex stage 0 at slot zero with one attached R8 constant buffer and aligned offset: fragment color and vertex XY-offset bindings are exactly 16 bytes, while a vertex `DP4` matrix is exactly 64 bytes. A zero range clears the selected stage slot.
...
INV_SRC_ALPHA`) or a blend-disabled nonzero RGBA mask word in bits 27–30, limited
...
At draw validation Rust snapshots selected 16-byte color/offset or 64-byte
matrix constant ranges and a bounded position list from attached one-to-three
VBO sources, directly or through bounded index-buffer lookups, then expands a
strip or fan before validation and packet construction.
Each source position must be finite, have `x`, `y`, and `z` in `[-1, 1]`, `w == 1`, and every consecutive triple must form a nondegenerate triangle. Vertex UBO offset forms additionally snapshot `[dx, dy, 0, 0]`, with finite `dx/dy` in `[-1, 1]`, translate local copied vertices, and repeat that validation before packet construction; the generic form retains one fixed UV varying. The `DP4` form snapshots sixteen finite stage-zero constants from an inline or exact 64-byte resource binding, evaluates four row-major dot products against each copied position, requires finite positive output `w`, divides to canonical coordinates, and rejects any vertex outside the already supported clip volume; no clipping repair occurs. It keeps that transformed snapshot for CPU replay. The exact non-depth solid singleton retains raw 16-byte positions plus the same rows for v15. The exact non-depth generic-RGBA passthrough singleton, with no fragment-constant multiply, retains raw 32-byte position/RGBA vertices for v16. The exact non-depth generic-UV one-texture singleton, with no fragment-constant modulation, retains raw 24-byte position/UV vertices plus one sampler snapshot for v17; the exact generic-UV two-texture multiply singleton retains the same raw vertices plus two independent sampler snapshots for v18. Browser validation repeats the projection boundary, checks v16 normalized finite RGBA, and checks v17/v18 bounded UV/sampler state before WebGPU row-dot execution. Its zero, one, or two exact generic `MOV` outputs retain the fixed color/UV bytes while the position is replaced. The resulting `w` is one before the existing CPU packet and raster paths.
...
durable producer is still owned by the same context; no CPU snapshot is made.
...
Solid color, interpolated vertex color, sampled texels, and texture-times-interpolated-color use source-over unless a uniform nonzero opaque channel mask is selected.
...
Schema 7 is `96 + 32N` bytes with position/RGBA vertices plus viewport/scissor. Schema 8 is `108 + 40N + 4WH` bytes with position/RGBA/UV vertices, viewport/scissor, one exact sampler, and one `W,H <= 64` BGRA texture. Schema 9 is legacy clear-one `LESS`; schema 10 carries a write-enabled comparison; schema 11 carries read-only solid DSA; schema 12 is schema 7 plus clear-one/canonical DSA vertex-color depth; schema 13 is `116 + 24N + 4WH` bytes with exact sampler, texture, clear-one, and canonical DSA one-texture depth; schema 14 is `116 + 40N + 4WH` bytes with the same depth tail and fixed texture-times-RGBA attributes. Schema 15 is `160 + 16N` bytes: colors, sixteen row-major matrix f32 values, raw position vertices, and viewport/scissor; it admits only the bounded non-depth solid DP4 form. Schema 16 is `160 + 32N` bytes: clear color, zero reserved draw color, the same matrix rows, and raw position/RGBA vertices; it admits only the exact bounded non-depth generic-RGBA-passthrough DP4 form. Schema 17 is `172 + 24N + 4WH` bytes: clear color, zero reserved draw color, matrix rows, raw position/UV vertices, viewport/scissor, one exact sampler, and one `W,H <= 64` BGRA texture; it admits only the exact bounded non-depth generic-UV one-texture DP4 form. Schema 18 is `184 + 24N + 4(W1H1 + W2H2)` bytes: the same raw position/UV and matrix fields, plus two exact samplers and two `W,H <= 64` BGRA textures; it admits only the exact bounded non-depth generic-UV two-texture-multiply DP4 form. The browser retains schema-1 parsing only for old packets, independently validates schemas 2–18,
...
See [VirGL validation evidence](virgl-validation.md) for Rust/browser coverage and
the native Linux transport harness, including its unverified guest phase.

## Next compatibility milestones

1. Obtain native end-to-end execution of the newly wired `VGM1` guest phase once shell startup is available, then extend the validated v15–v18 matrix lanes to explicitly checked texture-color forms rather than add exact TGSI strings.
