...
clear and one deliberately bounded clear-plus-triangle draw path.
...
| VirGL stream | Surface/framebuffer, canonical TGSI, vertex state, clear, and one `DRAW_VBO` | No arbitrary TGSI or fixed-function state |
| Presentation | A clear or one triangle is rendered through WebGPU | No composition, depth, blending, or textures |
...
The guest stream uses ordinary VirGL headers, object types 4, 5, and 7,
...
validates the envelope, reuses a WebGPU `float32x4` triangle pipeline, vertex
buffer, uniform buffer, and bind group for the device generation, then submits
one clear-and-draw render pass and waits for `GPUQueue.onSubmittedWorkDone()`.
...
storage, rasterizes the same bounded triangle, and emits ordinary scanout
damage. Thus `TRANSFER_FROM_HOST_3D` sees a defined result after completion;
...
instanced draws, multiple attributes, blending, depth/stencil, textures,
multi-target rendering, general readback, or a broad VirGL renderer.
...
Rust tests prove the capset bits, transactional no-clear rejection, standard
state setup, one `VGD1` payload, deferred acknowledgment, BGRA clear result,
CPU triangle raster result, and normal `WBGF` damage. Browser tests prove
private-envelope framing, malformed color/position rejection, one cached
WebGPU pipeline, two buffers, no depth texture, a `draw(3)`, and completion
only after the queue resolves.
