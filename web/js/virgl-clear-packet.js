...
export function parseVirglClearPacket(packet) {
  ...
  if (packet.byteLength !== PACKET_BYTES) throw new Error("VirGL clear packet has invalid length");
  ...
  if (version !== 1) throw new Error(`Unsupported VirGL clear packet version ${version}`);
  ...
  return {
    ...
    presentationLabel: "VirGL capset 1 clear", protocol: "virgl-clear", sequence, version,
  };
}
