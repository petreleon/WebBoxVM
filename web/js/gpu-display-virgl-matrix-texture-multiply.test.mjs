...
test("standard VirGL matrix dual textures multiply after GPU DP4 position transforms", async () => {
  ...
  assert.deepEqual(await display.present3d(virglMatrixTextureMultiplyPacket({ matrix, rightSampler: 0x1080, sequence: 98 })), { sequence: 98, success: true });
  ...
  assert.equal(device.bindGroups[0].entries.length, 5); assert.match(device.shaderModules[0].code, /dot\(scene\.matrix\[0\]/); assert.match(device.shaderModules[0].code, /\* textureSampleLevel/);
  assert.deepEqual(device.samplers.map(({ addressModeU }) => addressModeU), ["clamp-to-edge", "repeat"]);
  assert.deepEqual([...device.writes[0].data.subarray(0, 4)], [100, 100, 100, 255]); assert.deepEqual([...device.writes[1].data.subarray(0, 4)], [128, 128, 128, 255]);
  assert.equal(status.dataset.threeDAcceleration, "webgpu-virgl-capset1-matrix-texture-multiply");
});
