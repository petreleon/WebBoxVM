...
export class VirglMatrixVertexColorRenderer {
  #bindGroup; #bufferUsage; #generation = 0; #pipeline; #revision = 0; #session; #uniformBuffer;
  #vertexBuffer; #vertexCapacity = 0;
  ...
  async #ensurePipeline(backend) {
    ...
    const module = device.createShaderModule({ code: SHADER, label: "VirGL matrix vertex-color shader" });
    this.#pipeline = await captureWebGpuErrors(device, () => device.createRenderPipelineAsync({
      ...
      label: "VirGL matrix vertex-color pipeline", layout: "auto", primitive: { topology: "triangle-list" },
    ...
    await captureWebGpuErrors(device, () => {
      this.#uniformBuffer = device.createBuffer({ label: "VirGL matrix vertex-color rows", size: 64, usage: this.#bufferUsage.COPY_DST | this.#bufferUsage.UNIFORM });
  ...
  #issueDraw(backend, frame) {
    ...
    [this.#vertexBuffer, this.#vertexCapacity] = ensureBuffer(device, this.#vertexBuffer, this.#vertexCapacity,
      frame.vertices.byteLength, "VirGL matrix vertex-color attributes", this.#bufferUsage.COPY_DST | this.#bufferUsage.VERTEX);
    ...
    const encoder = device.createCommandEncoder({ label: "VirGL matrix vertex-color encoder" });
    ...
    }], label: "VirGL matrix vertex-color pass" });
