...
import { renderVirglClear } from "./webgpu-virgl-clear.js?v=20260904-virgl-readback-pool-r1"; import { VirglDrawRenderer } from "./webgpu-virgl-draw.js?v=20260904-virgl-readback-pool-r1"; import { VirglMatrixRenderer } from "./webgpu-virgl-matrix.js?v=20260904-virgl-readback-pool-r1"; import { VirglMatrixVertexColorRenderer } from "./webgpu-virgl-matrix-vertex-color.js?v=20260904-virgl-readback-pool-r1";
...
import { VirglTextureRenderer } from "./webgpu-virgl-texture.js?v=20260904-virgl-readback-pool-r1"; import { VirglMatrixTextureRenderer } from "./webgpu-virgl-matrix-texture.js?v=20260904-virgl-readback-pool-r1";
import { VirglTextureMultiplyRenderer } from "./webgpu-virgl-texture-multiply.js?v=20260904-virgl-readback-pool-r1"; import { VirglMatrixTextureMultiplyRenderer } from "./webgpu-virgl-matrix-texture-multiply.js?v=20260904-virgl-readback-pool-r1";
...
export class ExperimentalWebGpu3dRenderer {
  ...
  #vertexBuffer;
  #vertexCapacity = 0;
  #virglDraw; #virglMatrix; #virglMatrixVertexColor; #virglMatrixTexture; #virglMatrixTextureMultiply; #virglSolidBatch; #virglOutputs; #virglDepthBatch; #virglMaterialBatch; #virglDepth; #virglDepthTexture; #virglDepthTextureColor; #virglTexture; #virglTextureMultiply; #virglVertexColor; #virglTextureColor;
  constructor(session, options = {}) {
    ...
    [this.#virglDraw, this.#virglMatrix, this.#virglMatrixVertexColor, this.#virglMatrixTexture, this.#virglSolidBatch, this.#virglDepthBatch, this.#virglMaterialBatch, this.#virglDepth, this.#virglDepthTexture, this.#virglDepthTextureColor, this.#virglTexture, this.#virglTextureMultiply, this.#virglVertexColor, this.#virglTextureColor] = [
      new VirglDrawRenderer(session, options), new VirglMatrixRenderer(session, options), new VirglMatrixVertexColorRenderer(session, options), new VirglMatrixTextureRenderer(session, options), new VirglSolidBatchRenderer(session, options, this.#virglOutputs), new VirglDepthBatchRenderer(session, options), new VirglMaterialBatchRenderer(session, options, this.#virglOutputs), new VirglDepthRenderer(session, options), new VirglDepthTextureRenderer(session, options), new VirglDepthTextureColorRenderer(session, options),
    ...
    ]; this.#virglMatrixTextureMultiply = new VirglMatrixTextureMultiplyRenderer(session, options);
  }
  ...
  async render(backend, frame, isCurrent = () => true) {
    ...
    if (frame.protocol === "virgl-draw") return (frame.matrix ? this.#virglMatrix : this.#virglDraw).render(backend, frame, isCurrent);
    ...
    if (frame.protocol === "virgl-matrix-texture") return this.#virglMatrixTexture.render(backend, frame, isCurrent); if (frame.protocol === "virgl-texture") return this.#virglTexture.render(backend, frame, isCurrent);
    if (frame.protocol === "virgl-matrix-texture-multiply") return this.#virglMatrixTextureMultiply.render(backend, frame, isCurrent); if (frame.protocol === "virgl-texture-multiply") return this.#virglTextureMultiply.render(backend, frame, isCurrent);
    if (frame.protocol === "virgl-matrix-vertex-color") return this.#virglMatrixVertexColor.render(backend, frame, isCurrent); if (["virgl-vertex-color", "virgl-depth-vertex-color"].includes(frame.protocol)) return this.#virglVertexColor.render(backend, frame, isCurrent); if (frame.protocol === "virgl-texture-color") return this.#virglTextureColor.render(backend, frame, isCurrent);
  ...
  invalidate() {
    this.#revision += 1;
    this.#virglDraw.invalidate(); this.#virglMatrix.invalidate(); this.#virglMatrixVertexColor.invalidate(); this.#virglMatrixTexture.invalidate(); this.#virglMatrixTextureMultiply.invalidate(); this.#virglSolidBatch.invalidate(); this.#virglDepthBatch.invalidate(); this.#virglMaterialBatch.invalidate(); this.#virglDepth.invalidate(); this.#virglDepthTexture.invalidate(); this.#virglDepthTextureColor.invalidate(); this.#virglTexture.invalidate(); this.#virglTextureMultiply.invalidate(); this.#virglVertexColor.invalidate(); this.#virglTextureColor.invalidate();
