export function virglSolidBatchPacket({
  ...
  clearColor = [0, 0, 0, 1],
  version = 1,
  resident = [6, 7, 14, 15].includes(version),
  depthCompare = version === 2 ? 1 : 0,
  depthWriteEnabled = true,
  depthClear = [1, 6, 7, 8, 10, 12, 14, 15].includes(version) ? 0 : 1,
  drawCount = 2,
  draws = defaultDraws(canvasWidth, canvasHeight).slice(0, drawCount),
  residentPreviousProducer = [7, 15].includes(version) ? 72 : undefined,
  sequence = 73,
  writeMask = 9,
} = {}) {
  ...
  const headerBytes = [7, 15].includes(version) ? 52 : 48;
  const packet = new Uint8Array(headerBytes + body);
  packet.set([0x56, 0x47, 0x42, 0x31]);
  const view = new DataView(packet.buffer);
  [version, sequence, canvasWidth, canvasHeight, draws.length, version === 3 ? depthCompare : [6, 7].includes(version) && resident ? 1 : [12, 13, 14, 15].includes(version) ? writeMask : 0]
    .forEach((value, index) => view.setUint32(4 + index * 4, value, true));
  writeFloats(view, 28, clearColor);
  view.setFloat32(44, depthClear, true);
  if ([7, 15].includes(version)) view.setUint32(48, residentPreviousProducer, true);
