...
export class VirglTextureRenderer {
  #bindGroup;
  #bufferUsage;
  #generation = 0;
  #pipeline;
  ...
  async #ensurePipeline(backend) {
    ...
    const module = device.createShaderModule({ code: SHADER, label: "VirGL textured triangle shader" });
    this.#pipeline = await captureWebGpuErrors(device, () => device.createRenderPipelineAsync({
      ...
      label: "VirGL textured triangle pipeline", layout: "auto", primitive: { topology: "triangle-list" },
  ...
  #issueDraw(backend, frame) {
    ...
    }], label: "VirGL textured triangle pass" });
