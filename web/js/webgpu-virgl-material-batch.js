...
export class VirglMaterialBatchRenderer {
  ...
  async render(backend, frame, isCurrent) {
    ...
    try {
      ...
      const revision = this.#revision; output = this.#outputs.acquire(backend, frame);
    ...
    } catch (error) { this.#outputs.abandon(output); this.invalidate(); throw error; }
  }

  invalidate() {
    this.#outputs.invalidate(); this.#invalidateRenderer();
  }
  ...
  async #ensurePipelines(backend, frame) {
    if (this.#generation !== backend.deviceGeneration) { this.#invalidateRenderer(); this.#generation = backend.deviceGeneration; }
  ...
  #issueDraw(backend, frame, output) {
    ...
    if (output) {
      ...
      encoder.copyTextureToTexture({ texture: target }, { texture: canvas }, { depthOrArrayLayers: 1, height: frame.canvasHeight, width: frame.canvasWidth });
      device.queue.submit([encoder.finish()]);
      return Promise.resolve(device.queue.onSubmittedWorkDone()).finally(() => retired.forEach((texture) => texture.destroy?.()));
    }
