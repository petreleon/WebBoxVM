...
test("ensureWasm memoizes concurrent initialization", async () => {
  ...
  const { ensureWasm } = await import(`./wasm.js?v=20260904-virgl-material-batch-r1&memoized=${Date.now()}`);
...
test("threaded package failure falls back once to the serial package", async () => {
  ...
  const { ensureWasm } = await import(`./wasm.js?v=20260904-virgl-material-batch-r1&fallback=${Date.now()}`);
