...
/** Private WBG3 fallback/lifecycle owner, intentionally separate from VirGL routing. */
export class LegacyWbg3Renderer {
  #bindGroup; #bufferUsage; #canvasHeight = 0; #canvasWidth = 0; #depthTexture;
  #generation = 0; #indexBuffer; #indexCapacity = 0; #pipeline; #revision = 0;
  #session; #textureUsage; #uniformBuffer; #vertexBuffer; #vertexCapacity = 0;
  ...
  invalidate() {
    this.#revision += 1;
    this.#uniformBuffer?.destroy?.(); this.#vertexBuffer?.destroy?.();
    this.#indexBuffer?.destroy?.(); this.#depthTexture?.destroy?.();
    this.#bindGroup = undefined; this.#uniformBuffer = undefined; this.#vertexBuffer = undefined;
    this.#indexBuffer = undefined; this.#depthTexture = undefined; this.#pipeline = undefined;
    this.#generation = 0; this.#vertexCapacity = 0; this.#indexCapacity = 0;
  ...
  #issueDraw(backend, frame) {
    ...
    if (frame.indices.byteLength) device.queue.writeBuffer(this.#indexBuffer, 0, paddedIndexBytes(frame.indices));
    ...
    pass.setVertexBuffer(0, this.#vertexBuffer); pass.setIndexBuffer(this.#indexBuffer, "uint16");
    pass.drawIndexed(frame.indexCount); pass.end();
    ...
    this.#depthTexture = backend.device.createTexture({
      format: "depth24plus", label: "experimental guest 3D depth",
