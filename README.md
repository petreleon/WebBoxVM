...
- **Guest 3D paths** — [limited standard VirGL capset 1 copy/clear/transfers](research/virgl-compatibility.md) validates `RESOURCE_CREATE_3D`, conservative capset-context upload/readback, an attached off-screen `RESOURCE_COPY_REGION`, surface/framebuffer binding, and color clears before deferred WebGPU acknowledgment, with a native Linux guest KMS/transfer/fence proof; separately, [private capset 7](research/webgpu-acceleration.md) carries bounded `WBG3` geometry. Neither is Mesa/OpenGL or Vulkan/Venus compatibility yet.
