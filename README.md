...
- **Guest 3D paths** — [limited standard VirGL capset 1](research/virgl-compatibility.md) validates resource creation, transfer/readback, attached off-screen copy, surface/framebuffer binding, color clear, and one bounded standard clear-plus-triangle `DRAW_VBO` route that waits for WebGPU acknowledgment; the native Linux guest proof currently covers transport/copy/clear/fence, while private capset 7 carries separate bounded `WBG3` geometry. Neither route is Mesa/OpenGL or Vulkan/Venus compatibility yet.
