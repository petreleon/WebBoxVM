...
- **Guest 3D paths** — [limited standard VirGL capset 1](research/virgl-compatibility.md) validates resource creation, transfer/readback, attached off-screen copy, surface/framebuffer binding, color clear, exact source-over blending, and bounded canonical-TGSI solid, fragment-inline-constant, or resource-backed-fragment-constant, interpolated vertex-color, sampled, texture-times-vertex-color, or independently sampled two-texture `DRAW_VBO` paths. It resolves fixed u16/u32 indexes plus one-to-three standard VBO sources and normalizes `TRIANGLES`, alternating-winding `TRIANGLE_STRIP`, or spoke-preserving `TRIANGLE_FAN` input to bounded lists, with rasterizer, viewport, scissor, and canonical DSA depth compare/write-mask state for solid, vertex-color, and one-texture singleton routes; native-list and real-WebGPU proofs verify matching clipped pixels for their exercised routes after deferred completion. Private capset 7 carries separate bounded `WBG3` geometry. Neither route is Mesa/OpenGL or Vulkan/Venus compatibility yet.
