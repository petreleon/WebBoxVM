...
- **Guest 3D paths** — [limited standard VirGL capset 1](research/virgl-compatibility.md) validates resource creation, transfer/readback, attached off-screen copy, surface/framebuffer binding, color clear, exact source-over blending, and bounded solid or nearest-sampled-texture `DRAW_VBO` paths with standard rasterizer, viewport, and scissor state; the native guest and real-WebGPU proofs verify matching clipped pixels after deferred completion. Private capset 7 carries separate bounded `WBG3` geometry. Neither route is Mesa/OpenGL or Vulkan/Venus compatibility yet.
