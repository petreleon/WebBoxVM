...
- **Guest 3D paths** — [limited standard VirGL capset 1](research/virgl-compatibility.md) validates resource creation, transfer/readback, attached off-screen copy, surface/framebuffer binding, color clear, exact source-over blending, and bounded canonical-TGSI solid, interpolated vertex-color, sampled, texture-times-vertex-color, or independently sampled two-texture `DRAW_VBO` paths, including fixed u16/u32 index resolution and one-to-three standard vertex-buffer sources, with rasterizer, viewport, scissor state; native-guest and real-WebGPU proofs verify matching clipped pixels after deferred completion. Private capset 7 carries separate bounded `WBG3` geometry. Neither route is Mesa/OpenGL or Vulkan/Venus compatibility yet.
