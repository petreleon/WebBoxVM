...
- **Guest 3D paths** — [limited standard VirGL capset 1](research/virgl-compatibility.md) validates resource creation, transfer/readback, attached off-screen copy, surface/framebuffer binding, color clear, exact source-over blending, and bounded canonical-TGSI solid, interpolated vertex-color, sampled, texture-times-vertex-color, or independently sampled two-texture `DRAW_VBO` paths. It resolves fixed u16/u32 indexes plus one-to-three standard VBO sources and normalizes `TRIANGLES` or alternating-winding `TRIANGLE_STRIP` input to bounded lists, with rasterizer, viewport, and scissor state; native-list and real-WebGPU proofs verify matching clipped pixels after deferred completion. Private capset 7 carries separate bounded `WBG3` geometry. Neither route is Mesa/OpenGL or Vulkan/Venus compatibility yet.
