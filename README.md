...
- **Guest 3D paths** — [limited standard VirGL capset 1](research/virgl-compatibility.md) validates resource creation, transfer/readback, attached off-screen copy, surface/framebuffer binding, color clear, one exact standard source-over blend state, and one bounded standard clear-plus-triangle `DRAW_VBO` route that waits for WebGPU acknowledgment; the native Linux guest proof covers the exact shader/VBO/draw envelope, ordered deferred fences, and post-ack triangle readback, while private capset 7 carries separate bounded `WBG3` geometry. Neither route is Mesa/OpenGL or Vulkan/Venus compatibility yet.
